...
Extracts sixteen 168-hour seasonal windows from /nrel/wtk-us.h5 via HSDS/h5pyd,
using the same 500×500 spatial crop centered on 39.5°N 75°W as the 168-sample
benchmark.  Produces 2688 samples (16 windows × 168 hours) that are strictly
non-overlapping with the original benchmark (WTK indices 0..167).

Windows (four per season):
  winter_1: 336..503    winter_2: 504..671    winter_3: 672..839    winter_4: 840..1007
  spring_1: 2160..2327  spring_2: 2328..2495  spring_3: 2496..2663  spring_4: 2664..2831
  summer_1: 4344..4511  summer_2: 4512..4679  summer_3: 4680..4847  summer_4: 4848..5015
  fall_1:   6552..6719  fall_2:   6720..6887  fall_3:   6888..7055  fall_4:   7056..7223

HSDS endpoint and API key are never hardcoded.  They are read (in priority
order) from environment variables HS_ENDPOINT / HS_API_KEY, or from ~/.hscfg.
The developer.nlr.gov endpoint can be set via HS_ENDPOINT without modifying
this file.
...
Required output (in --out-dir):
  ...
  hr_stack.npy          (2688, 500, 500, 2) float64
  mr_stack.npy          (2688, 100, 100, 2) float64
  lr_stack.npy          (2688,  10,  10, 2) float64
...
_PROTECTED_PATHS = [
    ...
    'example_data_topology_expanded_672',   # 672-sample dataset
    'example_data_topology_expanded_1344',  # 1344-sample dataset
]

_DEFAULT_WTK_PATH = '/nrel/wtk-us.h5'
_DEFAULT_WINDOWS  = (
    'winter_1:336:168,winter_2:504:168,winter_3:672:168,winter_4:840:168,'
    'spring_1:2160:168,spring_2:2328:168,spring_3:2496:168,spring_4:2664:168,'
    'summer_1:4344:168,summer_2:4512:168,summer_3:4680:168,summer_4:4848:168,'
    'fall_1:6552:168,fall_2:6720:168,fall_3:6888:168,fall_4:7056:168'
)
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument('--windows', default=_DEFAULT_WINDOWS,
                    help='Comma-separated windows as name:start_idx:length '
                         f'(default: 16-window 2688-sample config).')
...
def _parse_windows(s: str) -> List[Tuple[str, int, int]]:
    """Parse "winter_1:336:168,winter_2:504:168" → [(name, start, length), ...]."""
    result = []
    for part in s.split(','):
...
def _write_notes(
    ...
    notes_path = REPO_ROOT / 'docs' / 'expanded_dataset_2688_notes.md'
    ...
    lines = [
        ...
        '## Summary',
        '',
        f'- Total samples: {n} (16 windows × 168 hours)',
    ...
    lines += [
        ...
        'None of the 2688 samples overlap the original benchmark (WTK 0..167 = '
        ...
        f'| `{out_dir}/hr_stack.npy` | (2688, 500, 500, 2) float64 |',
        f'| `{out_dir}/mr_stack.npy` | (2688, 100, 100, 2) float64 |',
        f'| `{out_dir}/lr_stack.npy` | (2688, 10, 10, 2) float64 |',
        f'| `{out_dir}/manifest.csv` | Per-sample metadata (2688 rows) |',
...
def main() -> None:
    ...
    print('=' * 64)
    print('PhIRE expanded dataset builder (2688-sample)')
    print('=' * 64)
    ...
    print()
    for name, start, length in windows:
        print(f'  [{name:10s}] t={start}..{start + length - 1}  ({length} samples)')
