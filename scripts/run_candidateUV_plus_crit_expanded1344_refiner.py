#!/usr/bin/env python3
"""
Candidate UV+crit-expanded-1344: scale-up of the completed
candidateUV_plus_crit_expanded672 run to 1344 training samples, using the
...
and Candidate C-expanded-1344 do -- no custom training loop is needed here
since there is no extra per-sample constraint data to feed), same 1344-sample
expanded seasonal training data as Candidate C-expanded-1344, same pretrained
...
scalar-speed gains. This 1344-sample run tests whether that conclusion holds
at a larger training scale:
  - If UV+crit remains topology-weak at 1344 samples, Candidate C's PD
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateUV_plus_crit_expanded1344_refiner.py

Next steps (not run by this script):
  Cheap scalar/domain evaluation:
    python3 scripts/evaluate_finetune_candidate.py \\
      --candidate-name candidateUV_plus_crit_expanded1344 \\
      --candidate-dir  data_out/wind_finetune_candidateUV_plus_crit_expanded1344 \\
      ...
      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded1344_eval
  ...
  comparison report are needed for this ablation):
    bash scripts/run_candidate_topology_pipeline.sh \\
      --method     candidateUV_plus_crit_expanded1344 \\
      --data-dir   data_out/wind_finetune_candidateUV_plus_crit_expanded1344 \\
      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded1344_topology_vti \\
      --out-base   ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded1344_topology \\
      --n-samples  168 \\
      --threads    1 \\
      --skip-viz
"""
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateUV_plus_crit_expanded1344.log'
...
# Training data: the exact same expanded 1344-sample seasonal dataset used by
# Candidate C-expanded-1344 -- reused unchanged.
...
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateUV_plus_crit_expanded1344'
DATA_OUT  = 'data_out/wind_finetune_candidateUV_plus_crit_expanded1344'
...
_PROTECTED = [
    ...
    # The sibling 2688-scale UV+crit script's output -- defense in
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate UV+crit-expanded-1344 fine-tuning (Candidate B scaffold ablation)')
    print('=' * 64)
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateUV_plus_crit_expanded1344',
    ...
    print('      --candidate-name candidateUV_plus_crit_expanded1344 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateUV_plus_crit_expanded1344 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded1344_eval')
    print()
    ...
    print('      --method     candidateUV_plus_crit_expanded1344 \\')
    print('      --data-dir   data_out/wind_finetune_candidateUV_plus_crit_expanded1344 \\')
    print('      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded1344_topology_vti \\')
    print('      --out-base   ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded1344_topology \\')
    print('      --n-samples  168 \\')
    print('      --threads    1 \\')
    print('      --skip-viz')
    print('=' * 64)
