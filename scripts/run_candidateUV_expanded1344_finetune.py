...
Scientific motivation:
  ...
  Both train on the same 1344-sample dataset; the only difference is that
  C-expanded-1344 uses the full Candidate C auxiliary loss suite (L_speed,
  L_grad, L_levelset, L_crit) while UV-expanded-1344 uses L_uv only.
  Comparing the two isolates whether C's topology-aware proxy losses provide
  gains beyond simply training on more data.

  Key comparisons:
    candidateUV_expanded1344 vs candidateC_expanded1344  — effect of C's auxiliary losses at 1344 samples
    candidateUV_expanded1344 vs candidateUV_expanded672  — effect of doubled training data (UV-only)
    candidateUV_expanded1344 vs CNN baseline             — effect of UV-only fine-tuning at 1344 samples
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateUV_expanded1344_finetune.py \\
    2>&1 | tee logs/wind_finetune_candidateUV_expanded1344.log
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateUV_expanded1344.log'
...
# Outputs
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateUV_expanded1344'
DATA_OUT  = 'data_out/wind_finetune_candidateUV_expanded1344'
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate UV-expanded-1344 fine-tuning (UV-only ablation)')
    print('=' * 64)
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateUV_expanded1344',
        learning_rate= 1e-5,
        N_epochs     = 3,
        save_every   = 1,
        print_every  = 2,
    ...
    print('Phase 1: UV-only fine-tuning on 1344-sample expanded dataset …')
    ...
    print('      --candidate-name candidateUV_expanded1344 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateUV_expanded1344 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_expanded1344_eval')
    print()
    print('  TTK topology pipeline:')
    print('    bash scripts/run_candidate_topology_pipeline.sh \\')
    print('      --method   candidateUV_expanded1344 \\')
    print('      --data-dir data_out/wind_finetune_candidateUV_expanded1344')
    print('=' * 64)
