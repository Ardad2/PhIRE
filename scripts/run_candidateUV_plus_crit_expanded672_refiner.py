#!/usr/bin/env python3
"""
Candidate UV+crit-expanded-672: ablation of Candidate C-expanded-672 with
Candidate B's scalar-speed, gradient, and level-set losses disabled, keeping
only Candidate C's high-speed local-maxima / critical-value proxy (L_crit)
on top of the bare normalized vector-reconstruction loss (L_uv). Tests
whether L_crit alone -- without Candidate B's scalar-field scaffold -- can
still produce topology-relevant training signal.
...
PhIREGANs.pretrain() directly, exactly like Candidate C-expanded-672 does --
no custom training loop is needed here since there is no extra per-sample
constraint data to feed), same 672-sample expanded seasonal training data,
same pretrained CNN initialization, same optimizer/epochs/batch size, same
output/inference shape conventions as Candidate C-expanded-672 -- the ONLY
intended difference is LAMBDA_SPEED/LAMBDA_GRAD/LAMBDA_LEVELSET being zeroed.
...
Loss configuration (Candidate UV + Candidate C's critical-value proxy only):
    ...
    L_speed, L_grad, and L_levelset are ALL EXPLICITLY DISABLED
    (LAMBDA_SPEED = LAMBDA_GRAD = LAMBDA_LEVELSET = 0.0), relative to
    Candidate C-expanded-672 (which used Candidate B's full scalar-field
    scaffold). This is the only loss-stack difference from Candidate C.
...
"Next priority: UV+crit-672"):
  - If UV+crit improves PD meaningfully, Candidate C's local-maxima
    critical-value proxy can produce topology signal without Candidate B.
  - If UV+crit fails while Candidate C succeeds, Candidate B's scalar-speed,
    gradient, and level-set losses are necessary scaffolding for L_crit.
  - If UV+crit improves PD but not MT, L_crit is likely a PD-oriented/
    local-extrema proxy rather than a merge-tree hierarchy proxy.
  - If UV+crit behaves like UV-only, Candidate C's success likely comes from
    the combination of Candidate B and L_crit, not from L_crit alone.
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateUV_plus_crit_expanded672_refiner.py

Next steps (not run by this script):
  Cheap scalar/domain evaluation:
    python3 scripts/evaluate_finetune_candidate.py \\
      --candidate-name candidateUV_plus_crit_expanded672 \\
      --candidate-dir  data_out/wind_finetune_candidateUV_plus_crit_expanded672 \\
      ...
      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded672_eval
  ...
  comparison report are needed for this ablation):
    bash scripts/run_candidate_topology_pipeline.sh \\
      --method     candidateUV_plus_crit_expanded672 \\
      --data-dir   data_out/wind_finetune_candidateUV_plus_crit_expanded672 \\
      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded672_topology_vti \\
      --out-base   ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded672_topology \\
      --n-samples  168 \\
      --threads    1 \\
      --skip-viz
"""
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateUV_plus_crit_expanded672.log'
...
# Training data: the exact same expanded 672-sample seasonal dataset used by
# Candidate C-expanded-672 -- reused unchanged.
...
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateUV_plus_crit_expanded672'
DATA_OUT  = 'data_out/wind_finetune_candidateUV_plus_crit_expanded672'
...
_PROTECTED = [
    ...
    # Completed native TF UV+E2-low-672 run -- closest sibling ablation
    # (bare L_uv + repaired TTK terms, no L_crit), must not be overwritten.
...
def _run_preflight_checks() -> None:
    ...
    print('  UV+crit-672: L_speed/L_grad/L_levelset disabled; only L_uv + L_crit active.')
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate UV+crit-expanded-672 fine-tuning (Candidate B scaffold ablation)')
    print('=' * 64)
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateUV_plus_crit_expanded672',
    ...
    print('      --candidate-name candidateUV_plus_crit_expanded672 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateUV_plus_crit_expanded672 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded672_eval')
    print()
    ...
    print('      --method     candidateUV_plus_crit_expanded672 \\')
    print('      --data-dir   data_out/wind_finetune_candidateUV_plus_crit_expanded672 \\')
    print('      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded672_topology_vti \\')
    print('      --out-base   ttk_runs_fixed/topology_finetuning/candidateUV_plus_crit_expanded672_topology \\')
    print('      --n-samples  168 \\')
    print('      --threads    1 \\')
    print('      --skip-viz')
    print('=' * 64)
