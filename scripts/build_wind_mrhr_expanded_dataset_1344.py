...
Extracts eight 168-hour seasonal windows from /nrel/wtk-us.h5 via HSDS/h5pyd,
using the same 500×500 spatial crop centered on 39.5°N 75°W as the 168-sample
benchmark.  Produces 1344 samples (8 windows × 168 hours) that are strictly
non-overlapping with the original benchmark (WTK indices 0..167).

Windows (two per season):
  winter_1: 336..503    winter_2: 504..671
  spring_1: 2160..2327  spring_2: 2328..2495
  summer_1: 4344..4511  summer_2: 4512..4679
  fall_1:   6552..6719  fall_2:   6720..6887
...
Required output (in --out-dir):
  ...
  hr_stack.npy          (1344, 500, 500, 2) float64
  mr_stack.npy          (1344, 100, 100, 2) float64
  lr_stack.npy          (1344,  10,  10, 2) float64
...
_PROTECTED_PATHS = [
    ...
    'example_data_topology_expanded_672',  # existing 672-sample dataset
]

_DEFAULT_WTK_PATH = '/nrel/wtk-us.h5'
_DEFAULT_WINDOWS  = (
    'winter_1:336:168,winter_2:504:168,'
    'spring_1:2160:168,spring_2:2328:168,'
    'summer_1:4344:168,summer_2:4512:168,'
    'fall_1:6552:168,fall_2:6720:168'
)
...
def _write_notes(
    ...
    notes_path = REPO_ROOT / 'docs' / 'expanded_dataset_1344_notes.md'
    ...
    lines = [
        ...
        '## Summary',
        '',
        f'- Total samples: {n} (8 windows × 168 hours)',
    ...
    lines += [
        ...
        'None of the 1344 samples overlap the original benchmark (WTK 0..167 = '
        ...
        f'| `{out_dir}/hr_stack.npy` | (1344, 500, 500, 2) float64 |',
        f'| `{out_dir}/mr_stack.npy` | (1344, 100, 100, 2) float64 |',
        f'| `{out_dir}/lr_stack.npy` | (1344, 10, 10, 2) float64 |',
        f'| `{out_dir}/manifest.csv` | Per-sample metadata (1344 rows) |',
...
def main() -> None:
    ...
    print('=' * 64)
    print('PhIRE expanded dataset builder (1344-sample)')
    print('=' * 64)
