...
TTK constraints are pre-computed by build_candidateE2_expanded672_constraints.py
...
Workflow:
  Phase 1 — Training:  train RefinerNet on 672 expanded CNN SR outputs
            ...
            Output: models_fixed/topology_finetuning/
                      wind_finetune_candidateE2_expanded672/
...
OUT_DIR  = REPO_ROOT / "data_out" / "wind_finetune_candidateE2_expanded672"
LOG_PATH = REPO_ROOT / "logs"    / "wind_finetune_candidateE2_expanded672.log"
    ...
    def __init__(self, npz_path: Path, patch: int, device: torch.device) -> None:
        ...
        if missing:
            raise ValueError(
                ...
                "Re-run build_candidateE2_expanded672_constraints.py."
            )
...
def _check_preconditions() -> None:
    ...
    if n_constraints != N_TRAIN:
        sys.exit(
            ...
            "  Re-run: python3 scripts/build_candidateE2_expanded672_constraints.py"
        )
