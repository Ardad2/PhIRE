...
def _parse_rows(text: Optional[str]) -> Optional[List[int]]:
    ...
    for tok in text.replace(",", " ").split():
        v = int(tok)
        if v < 0:
            raise SystemExit(f"Row index must be non-negative, got {v}")
        out.append(v)
    return sorted(set(out))


def parse_raw_records(path: Path, max_records: int, rows: Optional[Tuple[int, ...]] = None) -> List[RecordRaw]:
    ...
    for row, raw in enumerate(tf.compat.v1.io.tf_record_iterator(str(path))):
        ...
        if wanted is None and len(out) >= max_records:
            break
...
def main() -> int:
    ...
    ap.add_argument("--rows", default=None, help="Optional explicit TFRecord row indices (e.g. '101 134 162')")
    ...
    rows_tuple = tuple(selected_rows) if selected_rows is not None else None
    records = parse_raw_records(args.tfrecord, max_records=args.max_records, rows=rows_tuple)
    if not records:
        if selected_rows is not None:
            raise SystemExit(f"No selected rows found in {args.tfrecord}: {selected_rows}")
