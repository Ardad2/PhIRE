#!/usr/bin/env python3
"""
Candidate UV+E2-tf-lowlambda-expanded-672: ablation of
candidateB_plus_E2_tf_lowlambda_expanded672 with Candidate B's scalar-speed,
gradient, and level-set losses ALSO disabled (in addition to L_crit, which
B+E2 already disables), to test whether Candidate B's scalar-field proxy
losses are necessary for the repaired TTK fixed-index losses (L_ttkcv,
L_ttkpers) to produce a useful training signal on their own. Same
PhIRE/TensorFlow generator fine-tuning path (sr_network.SR_NETWORK), same
672-sample expanded seasonal training data, same pretrained CNN
initialization, same optimizer/epochs/batch size, same repaired E2
constraint file, same normalized L_uv convention, and same output/inference
shape conventions as candidateB_plus_E2_tf_lowlambda_expanded672 -- the ONLY
intended difference is LAMBDA_SPEED/LAMBDA_GRAD/LAMBDA_LEVELSET also being
zeroed (LAMBDA_CRIT was already zero in B+E2).
...
residual head) as Candidate C-expanded-672 and candidateB_plus_E2_tf_
...
Loss configuration (Candidate UV + repaired low-lambda TTK terms only):
    ...
    L_speed, L_grad, L_levelset, and L_crit are ALL EXPLICITLY DISABLED
    (LAMBDA_SPEED = LAMBDA_GRAD = LAMBDA_LEVELSET = LAMBDA_CRIT = 0.0),
    relative to candidateB_plus_E2_tf_lowlambda_expanded672 (which kept
    LAMBDA_SPEED/LAMBDA_GRAD/LAMBDA_LEVELSET at Candidate B's values) and
    candidateE2_tf_lowlambda_expanded672 (which additionally kept
    LAMBDA_CRIT=0.001). This is the only loss-stack difference from B+E2.
...
TTK constraints (identical file to candidateB_plus_E2_tf_lowlambda_expanded672
and candidateE2_tf_lowlambda_expanded672 -- same 672-sample training scale,
reused unchanged):
...
"Next priority: UV+E2-low-672"):
  - If UV+E2-low-672 improves MT meaningfully, the repaired TTK fixed-index
    losses can drive topology even without Candidate B's scalar scaffold.
  - If UV+E2-low-672 fails while B+E2-low succeeds, Candidate B's
    scalar-speed/gradient/level-set terms are necessary for the TTK
    critical-pair supervision to become useful.
  - If UV+E2-low improves MT but damages fidelity, Candidate B likely acts
    as a stabilizer rather than as the main topology signal.

Workflow:
  Phase 1 -- Training: fine-tune the pretrained vector [u,v] CNN for 3
             epochs on the 672-sample expanded dataset.
             Output: models_fixed/topology_finetuning/
                       wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672/
  Phase 2 -- Evaluation: paired inference on the 168-sample benchmark via
             PhIREGANs.test_paired(), unchanged from Candidate C.
             Output: data_out/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672/
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateUV_plus_E2_tf_lowlambda_expanded672_ttkcrit_refiner.py

Next steps (not run by this script):
  Cheap scalar/domain evaluation:
    python3 scripts/evaluate_finetune_candidate.py \\
      --candidate-name candidateUV_plus_E2_tf_lowlambda_expanded672 \\
      --candidate-dir  data_out/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672 \\
      ...
      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_plus_E2_tf_lowlambda_expanded672_eval
  ...
  the distances/comparison report are needed for this ablation):
    bash scripts/run_candidate_topology_pipeline.sh \\
      --method     candidateUV_plus_E2_tf_lowlambda_expanded672 \\
      --data-dir   data_out/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672 \\
      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateUV_plus_E2_tf_lowlambda_expanded672_topology_vti \\
      --out-base   ttk_runs_fixed/topology_finetuning/candidateUV_plus_E2_tf_lowlambda_expanded672_topology \\
      --n-samples  168 \\
      --threads    1 \\
      --skip-viz
"""
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672.log'
...
# Training data: same expanded 672-sample seasonal dataset as Candidate C-expanded-672
# and candidateB_plus_E2_tf_lowlambda_expanded672 -- reused unchanged.
...
# the exact same file used by candidateB_plus_E2_tf_lowlambda_expanded672 and
# candidateE2_tf_lowlambda_expanded672 (same 672-sample training scale).
CONSTRAINTS_NPZ = (
    REPO_ROOT / 'ttk_runs_fixed' / 'topology_finetuning'
    / 'candidateE2_fixed_constraints' / 'ttk_pd_critical_pairs_gtvalues.npz'
)

# Outputs -- new, distinct names; do not overwrite any existing candidate
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672'
DATA_OUT  = 'data_out/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672'
...
# ---------------------------------------------------------------------------
# Training hyperparameters (identical to Candidate C-expanded-672 /
# candidateB_plus_E2_tf_lowlambda_expanded672)
# ---------------------------------------------------------------------------
LEARNING_RATE = 1e-5
N_EPOCHS      = 3
SAVE_EVERY    = 1
...
def _run_preflight_checks(phire: PhIREGANs):
    ...
    if not CONSTRAINTS_NPZ.exists():
        sys.exit(
            ...
            '      --gt-path    data_out/wind_mrhr_cnn_expanded672/dataGT.npy \\\n'
            '      --idx-path   data_out/wind_mrhr_cnn_expanded672/idx.npy \\\n'
            '      --n-expected 672 \\\n'
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate UV+E2-tf-lowlambda-expanded-672 fine-tuning (Candidate B scaffold ablation)')
    print('=' * 64)
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672',
    ...
    # Identical to candidateB_plus_E2_tf_lowlambda_expanded672 -- TTK losses
    ...
    print('      --candidate-name candidateUV_plus_E2_tf_lowlambda_expanded672 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateUV_plus_E2_tf_lowlambda_expanded672_eval')
    print()
    ...
    print('      --method     candidateUV_plus_E2_tf_lowlambda_expanded672 \\')
    print('      --data-dir   data_out/wind_finetune_candidateUV_plus_E2_tf_lowlambda_expanded672 \\')
    print('      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateUV_plus_E2_tf_lowlambda_expanded672_topology_vti \\')
    print('      --out-base   ttk_runs_fixed/topology_finetuning/candidateUV_plus_E2_tf_lowlambda_expanded672_topology \\')
    print('      --n-samples  168 \\')
    print('      --threads    1 \\')
    print('      --skip-viz')
    print('=' * 64)
