#!/usr/bin/env python3
"""
Candidate B+E2-tf-lowlambda-expanded-672: ablation of
candidateE2_tf_lowlambda_expanded672 with L_crit removed (LAMBDA_CRIT=0.0),
to isolate whether that run's strong MT improvement came from the
high-speed local-maxima critical-value proxy (L_crit, Candidate C's term)
or from the repaired TTK fixed-index losses (L_ttkcv, L_ttkpers) added on
top of it. Same PhIRE/TensorFlow generator fine-tuning path
(sr_network.SR_NETWORK), same 672-sample expanded seasonal training data,
same pretrained CNN initialization, same optimizer/epochs/batch size, same
repaired E2 constraint file, same normalized L_uv convention, and same
output/inference shape conventions as candidateE2_tf_lowlambda_expanded672
-- the ONLY intended difference is L_crit being disabled.
...
residual head) as Candidate C-expanded-672 and candidateE2_tf_lowlambda_
...
Loss configuration (Candidate B stack + repaired low-lambda TTK terms):
    ...
    L_crit is EXPLICITLY DISABLED (LAMBDA_CRIT = 0.0) relative to
    candidateE2_tf_lowlambda_expanded672, which used 0.001 * L_crit. This is
    the only loss-stack difference between the two scripts.
...
Workflow:
  Phase 1 -- Training: fine-tune the pretrained vector [u,v] CNN for 3
             ...
             Output: models_fixed/topology_finetuning/
                       wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672/
  Phase 2 -- Evaluation: paired inference on the 168-sample benchmark via
             PhIREGANs.test_paired(), unchanged from Candidate C.
             Output: data_out/wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672/
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateB_plus_E2_tf_lowlambda_expanded672_ttkcrit_refiner.py
...
Next steps (not run by this script):
  ...
  the distances/comparison report are needed for this ablation):
    bash scripts/run_candidate_topology_pipeline.sh \\
      ...
      --data-dir   data_out/wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672 \\
      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateB_plus_E2_tf_lowlambda_expanded672_topology_vti \\
      --out-base   ttk_runs_fixed/topology_finetuning/candidateB_plus_E2_tf_lowlambda_expanded672_topology \\
      --n-samples  168 \\
      --threads    1 \\
      --skip-viz
"""
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672.log'
...
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672'
DATA_OUT  = 'data_out/wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672'
...
_PROTECTED = [
    ...
    # ablation ("B + E2, L_crit removed") is comparing against -- must not
    # be overwritten by this sibling script.
...
# ---------------------------------------------------------------------------
# Candidate B loss weights: identical to Candidate C-expanded-672 except
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate B+E2-tf-lowlambda-expanded-672 fine-tuning (L_crit ablation)')
    print('=' * 64)
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672',
    ...
    # Identical to Candidate C-expanded-672 -- TTK losses are training-time
    ...
    print('      --candidate-name candidateB_plus_E2_tf_lowlambda_expanded672 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateB_plus_E2_tf_lowlambda_expanded672_eval')
    print()
    ...
    print('      --method     candidateB_plus_E2_tf_lowlambda_expanded672 \\')
    print('      --data-dir   data_out/wind_finetune_candidateB_plus_E2_tf_lowlambda_expanded672 \\')
    print('      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateB_plus_E2_tf_lowlambda_expanded672_topology_vti \\')
    print('      --out-base   ttk_runs_fixed/topology_finetuning/candidateB_plus_E2_tf_lowlambda_expanded672_topology \\')
    print('      --n-samples  168 \\')
    print('      --threads    1 \\')
    print('      --skip-viz')
    print('=' * 64)
