#!/usr/bin/env python3
"""Evaluate Candidate B fine-tuned CNN against baselines.

Computes per-sample metrics for: bicubic (optional), baseline CNN, baseline GAN,
and Candidate B fine-tuned CNN.  Writes:

  <out-dir>/all_sample_metrics_candidateB.csv   — one row per (sample, method)
  <out-dir>/pairwise_cnn_vs_candidateB.csv      — per-metric delta CNN vs CandidateB
  <out-dir>/winner_counts.csv                   — wins per method per metric
  <out-dir>/adjacent_cluster_table.csv          — detail for adjacent-cluster samples
  docs/topology_finetuning_candidateB_eval.md   — markdown report (7 questions answered)
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument('--candidate-dir', required=True,
                    help='Directory with Candidate B outputs (dataSR/GT/IN/idx.npy)')
    ...
    ap.add_argument('--out-dir',
                    default='ttk_runs_fixed/topology_finetuning/candidateB_eval',
                    help='Output directory')
    ap.add_argument('--report-path',
                    default='docs/topology_finetuning_candidateB_eval.md',
...
def main() -> None:
    ...
    idx_arrays = {'cnn': cnn_idx, 'gan': gan_idx, 'candidateB': cand_idx}
    ...
    cand_pos_c = pos_maps['candidateB']
    ...
    methods_to_run.append(('gan',
                           _slice(cnn_gt, 'cnn'),
                           _slice(gan_sr, 'gan')))
    methods_to_run.append(('candidateB',
                           _slice(cnn_gt, 'cnn'),
                           _slice(cand_sr, 'candidateB')))
    ...
    for method, gt4d, sr4d in methods_to_run:
        ...
        for i, si in enumerate(common_idx):
            ...
            row: Dict = {
                ...
                # Topology distances (from merged CSV; NaN for candidateB/bicubic)
                'pd_distance':        pd_dist,
                'mt_distance':        mt_dist,
            }
            all_rows.append(row)
    ...
    # -----------------------------------------------------------------------
    csv_path = out_dir / 'all_sample_metrics_candidateB.csv'
    ...
    _write_csv(out_dir / 'winner_counts.csv', winner_rows)
    print(f'Written: {out_dir}/winner_counts.csv')

    # -----------------------------------------------------------------------
    # Pairwise comparison: baseline CNN vs Candidate B
    # -----------------------------------------------------------------------
    pairwise_rows: List[Dict] = []
    cnn_d  = method_data.get('cnn', {})
    cand_d = method_data.get('candidateB', {})
    ...
    for metric in metric_cols:
        ...
        mu_cnn  = _fmt(vals_cnn)
        mu_cand = _fmt(vals_cand)
    ...
    _write_csv(out_dir / 'pairwise_cnn_vs_candidateB.csv', pairwise_rows)
    print(f'Written: {out_dir}/pairwise_cnn_vs_candidateB.csv')
    ...
    for si in ADJACENT_CLUSTER_SAMPLES:
        ...
        # Derived: delta fields (candidateB vs CNN) for key metrics
    ...
    _write_csv(out_dir / 'adjacent_cluster_table.csv', adj_rows)
    print(f'Written: {out_dir}/adjacent_cluster_table.csv')
    ...
    # -----------------------------------------------------------------------
    md = io.StringIO()
    md.write('# Candidate B fine-tuning evaluation\n\n')
    ...
    # Q1: Direct fidelity
    md.write('### Q1. Did Candidate B preserve CNN\'s direct-fidelity advantage?\n\n')
    for metric, lower_better, label in [
        ...
        v_cand = _mean_for('candidateB', metric)
        if 'gan' in method_data:
            v_gan = _mean_for('gan', metric)
            md.write(f'- **{label}**: CNN={_fmt(v_cnn)}, GAN={_fmt(v_gan)}, '
                     f'CandidateB={_fmt(v_cand)}. '
                     f'Δ(CandidateB−CNN)={_delta_str("cnn","candidateB",metric,lower_better)}, '
                     f'improved on {_n_improved(metric)} samples.\n')
        else:
            md.write(f'- **{label}**: CNN={_fmt(v_cnn)}, '
                     f'CandidateB={_fmt(v_cand)}. '
                     f'Δ={_delta_str("cnn","candidateB",metric,lower_better)}.\n')
    md.write('\n')

    # Q2: Speed/WPD
    md.write('### Q2. Did Candidate B improve scalar speed or WPD metrics?\n\n')
    for metric, lower_better, label in [
        ...
        v_cnn  = _mean_for('cnn', metric)
        v_cand = _mean_for('candidateB', metric)
        md.write(f'- **{label}**: CNN={_fmt(v_cnn)}, CandidateB={_fmt(v_cand)}. '
                 f'Δ={_delta_str("cnn","candidateB",metric,lower_better)}, '
                 f'improved on {_n_improved(metric)} samples.\n')
    md.write('\n')

    # Q3: Gradient
    md.write('### Q3. Did Candidate B improve gradient metrics?\n\n')
    for metric, lower_better, label in [
            ('grad_mae', True, 'Gradient MAE'),
            ('grad_w1',  True, 'Gradient W1'),
            ('grad_kurtosis_abs_delta', True, 'Gradient kurtosis abs Δ')]:
    ...
    # Q4: Exceedance
    md.write('### Q4. Did Candidate B improve exceedance metrics, especially s>5?\n\n')
    for metric, lower_better, label in [
            ('exceed_abs_t5',  True, 'Exceedance abs Δ s>5'),
            ('exceed_abs_t10', True, 'Exceedance abs Δ s>10'),
            ('exceed_abs_t15', True, 'Exceedance abs Δ s>15'),
        ...
        v_cnn  = _mean_for('cnn', metric)
        v_cand = _mean_for('candidateB', metric)
        md.write(f'- **{label}**: CNN={_fmt(v_cnn,5)}, CandidateB={_fmt(v_cand,5)}. '
                 f'Δ={_delta_str("cnn","candidateB",metric,lower_better)}, '
                 f'improved on {_n_improved(metric)} samples.\n')
    md.write('\n')

    # Q5: Component counts
    md.write('### Q5. Did Candidate B move component-count behavior toward GT?\n\n')
    for metric, lower_better, label in [
            ('comp_curve_l1', True, 'Component-count curve L1'),
    ...
    # Q6: PD/MT
    md.write('### Q6. Did Candidate B improve PD or MT distances?\n\n')
    md.write('PD and MT distances for Candidate B are not available in this '
             'evaluation (TTK was not re-run on pilot outputs). '
    ...
    for metric, lower_better, label in [
        ...
        md.write(f'- **{label}**: CNN={_fmt(v_cnn)}, GAN={_fmt(v_gan)}, '
                 f'CandidateB=N/A (requires TTK run)\n')
    md.write('\nTo compute PD/MT for Candidate B: convert outputs to VTI via '
             '`scripts/convert_phire_to_vti.py`, then run '
             '`scripts/compute_composite_tree_distance.py`.\n\n')

    # Q7: Adjacent clusters
    md.write('### Q7. Did Candidate B change the adjacent clusters '
             '(samples 10–13, 90–93, 162–163)?\n\n')
    md.write('See `adjacent_cluster_table.csv` for full detail. '
    ...
    md.write('## Pairwise summary: CNN vs Candidate B\n\n')
    md.write('| Metric | Mean CNN | Mean CandB | Δ | N improved | N worsened |\n')
    md.write('|---|---|---|---|---|---|\n')
    for pr in pairwise_rows:
        if any(pr['metric'] == m for m, _, _ in key_metrics):
            md.write(f'| {pr["metric"]} '
                     f'| {_fmt(pr["mean_baseline_cnn"],4)} '
                     f'| {_fmt(pr["mean_candidateB"],4)} '
                     f'| {_fmt(pr["delta"],4)} '
                     f'| {pr["n_improved"]} '
                     f'| {pr["n_worsened"]} |\n')
    md.write('\n')
    ...
    md.write('- PD/MT distances for CNN and GAN are loaded from pre-computed '
             'merged CSV; Candidate B requires a fresh TTK run.\n')
    md.write('- Candidate B was trained for 3 epochs from the pretrained CNN '
             'checkpoint — results may improve with longer training.\n')
