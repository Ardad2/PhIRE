#!/usr/bin/env python3
"""
Candidate E2-tf-lowlambda-expanded-1344: scale-up of the completed
candidateE2_tf_lowlambda_expanded672 run to 1344 training samples, using the
same loss configuration -- Candidate C's full stack (L_uv/L_speed/L_grad/
L_levelset/L_crit) plus repaired low-lambda TTK fixed-index losses
(L_ttkcv, L_ttkpers). LAMBDA_CRIT stays ENABLED at 0.001, unchanged from the
672 run -- this is Candidate C + repaired E2, NOT the B+E2 (L_crit-disabled)
ablation. This is an apples-to-apples scale comparison against Candidate
C-expanded-1344 and the completed candidateB_plus_E2_tf_lowlambda_expanded1344
run, following the same 672->1344->2688 training-scale ladder already used
for Candidate C, the B+E2-low ablation family, and the PyTorch-refiner
E2-low family.

Background (unchanged from the 672 script): L_crit is Candidate C's
high-speed local-maxima critical-value proxy. This script keeps it enabled
(LAMBDA_CRIT=0.001) so the full Candidate C stack is preserved at every
training scale; the L_crit-disabled ablation lives in the separate
candidateB_plus_E2_tf_lowlambda_expanded1344 script. Same PhIRE/TensorFlow
generator fine-tuning path (sr_network.SR_NETWORK), same pretrained CNN
initialization, same optimizer/epochs/batch size, same repaired-E2
constraint convention (rebuilt at this training scale), same normalized
L_uv convention, and same output/inference shape conventions as
...
This is "Candidate C loss stack + repaired TTK fixed-index losses": same
architecture (fine-tunes the actual generator, not a frozen-CNN post-hoc
residual head) as Candidate C-expanded-1344 and candidateE2_tf_lowlambda_
expanded672, with TTK fixed-index losses (L_ttkcv, L_ttkpers) added on top
of the full Candidate C loss stack (L_crit included).

Loss configuration (Candidate C stack + repaired low-lambda TTK terms):
    ...
    L_crit remains ENABLED (LAMBDA_CRIT = 0.001), identical to
    candidateE2_tf_lowlambda_expanded672 and Candidate C. It is NOT
...
sr_network.py or PhIREGANs.py. L_ttkcv/L_ttkpers are new TF ops added in this
script only, operating on model.x_SR (normalized predicted [u,v]) via the
same _denorm_speed() helper Candidate C's losses already use internally.
...
Workflow:
  Phase 1 -- Training: fine-tune the pretrained vector [u,v] CNN for 3
             ...
             Output: models_fixed/topology_finetuning/
                       wind_finetune_candidateE2_tf_lowlambda_expanded1344/
  Phase 2 -- Evaluation: paired inference on the 168-sample benchmark via
             PhIREGANs.test_paired(), unchanged from Candidate C.
             Output: data_out/wind_finetune_candidateE2_tf_lowlambda_expanded1344/
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateE2_tf_lowlambda_expanded1344_ttkcrit_refiner.py

Next steps (not run by this script):
  Cheap scalar/domain evaluation:
    python3 scripts/evaluate_finetune_candidate.py \\
      --candidate-name candidateE2_tf_lowlambda_expanded1344 \\
      --candidate-dir  data_out/wind_finetune_candidateE2_tf_lowlambda_expanded1344 \\
      ...
      --out-dir        ttk_runs_fixed/topology_finetuning/candidateE2_tf_lowlambda_expanded1344_eval
  ...
  the distances/comparison report are needed for this scale-up):
    bash scripts/run_candidate_topology_pipeline.sh \\
      --method     candidateE2_tf_lowlambda_expanded1344 \\
      --data-dir   data_out/wind_finetune_candidateE2_tf_lowlambda_expanded1344 \\
      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateE2_tf_lowlambda_expanded1344_topology_vti \\
      --out-base   ttk_runs_fixed/topology_finetuning/candidateE2_tf_lowlambda_expanded1344_topology \\
      --n-samples  168 \\
      --threads    1 \\
      --skip-viz
"""
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateE2_tf_lowlambda_expanded1344.log'
...
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateE2_tf_lowlambda_expanded1344'
DATA_OUT  = 'data_out/wind_finetune_candidateE2_tf_lowlambda_expanded1344'
...
_PROTECTED = [
    ...
    # The completed 672-sample Candidate E2-tf-lowlambda run this scale-up
    # ladder traces back to -- must not be overwritten by this or any
    # sibling script.
    ...
    # The completed B+E2-low (L_crit-disabled) ablation family at all three
    # scales -- distinct experiment, must not be overwritten.
    ...
    # The sibling 2688-scale C+E2 script's output/constraints -- defense
    # in depth against a copy-paste mistake between the two scale-up scripts.
...
# ---------------------------------------------------------------------------
# Candidate C loss weights (unchanged -- this is "Candidate C + repaired TTK")
# ---------------------------------------------------------------------------
LAMBDA_SPEED    = 0.01
LAMBDA_GRAD     = 0.05
LAMBDA_WPD      = 0.0
...
def _run_preflight_checks(phire: PhIREGANs):
    ...
    print(f'    LAMBDA_CRIT     = {LAMBDA_CRIT}  (ENABLED -- Candidate C stack retained, matches candidateE2_tf_lowlambda_expanded672)')
    print('  LAMBDA_CRIT=0.001 ENABLED: this is Candidate C + repaired E2, not B+E2.')
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate E2-tf-lowlambda-expanded-1344 fine-tuning')
    print('=' * 64)
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateE2_tf_lowlambda_expanded1344',
        ...
        diag_vals = sess.run(
            {
                ...
                'L_crit': model.loss_breakdown['L_crit'],
                'L_ttkcv': L_ttkcv,
                'L_ttkpers': L_ttkpers,
                'g_loss (Candidate C stack)': model.g_loss,
    ...
    # Identical to Candidate C-expanded-1344 -- TTK losses are training-time
    ...
    print('      --candidate-name candidateE2_tf_lowlambda_expanded1344 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateE2_tf_lowlambda_expanded1344 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateE2_tf_lowlambda_expanded1344_eval')
    print()
    ...
    print('      --method     candidateE2_tf_lowlambda_expanded1344 \\')
    print('      --data-dir   data_out/wind_finetune_candidateE2_tf_lowlambda_expanded1344 \\')
    print('      --vti-dir    ttk_runs_fixed/topology_finetuning/candidateE2_tf_lowlambda_expanded1344_topology_vti \\')
    print('      --out-base   ttk_runs_fixed/topology_finetuning/candidateE2_tf_lowlambda_expanded1344_topology \\')
    print('      --n-samples  168 \\')
    print('      --threads    1 \\')
    print('      --skip-viz')
    print('=' * 64)
