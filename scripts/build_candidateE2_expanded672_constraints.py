#!/usr/bin/env python3
"""
Build CandidateE2-expanded-672 TTK critical-pair constraints from GT numpy arrays.

CandidateE2 uses "TTK-style" birth/death vertex supervision: fixed vertex positions
identify critical points in the GT wind-speed field, and the refiner is trained to
reproduce the exact GT speed values at those vertices.

The "corrected" aspect of E2 (vs E1): target birth_val/death_val are read directly
from the GT numpy arrays at the stored vertex positions, not from TTK VTU files.
This guarantees exact match between stored targets and actual GT values.

This helper generates the constraints without requiring TTK to be run on the expanded
dataset.  Instead, local maxima of the GT speed field (160×160 crop) are detected
using scipy/numpy and paired with the global minimum of the same crop, approximating
the superlevel-set persistence diagram that TTK would compute.
...
NPZ keys (same format as TTKConstraints expects):
  n_samples     : int (672)
  ...
  birth_vid     : (P,) int64   — flat vertex IDs (y*PATCH + x) in 160×160 grid
  death_vid     : (P,) int64
  birth_val     : (P,) float32 — GT scalar speed at birth vertex (exact from numpy)
  death_val     : (P,) float32 — GT scalar speed at death vertex (exact from numpy)
  persistence   : (P,) float32 — |birth_val - death_val|

Parameters:
  PATCH            = 160  — crop size; vertex IDs use W=160
  TOP_K            = 64   — max pairs per sample (matches E pilot --top-k 64)
  PERSISTENCE_FRAC = 0.01 — min persistence as fraction of speed range (matches E pilot)
  MAX_FILTER_SIZE  = 5    — neighborhood for local maxima detection

Sanity check:
  After writing the NPZ, the script verifies that every stored birth_val/death_val
  matches the actual GT speed at the corresponding vertex within float32 tolerance
  (max absolute error < 1e-5).

Usage:
  python3 scripts/build_candidateE2_expanded672_constraints.py
