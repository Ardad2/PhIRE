#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Topology pipeline for Candidate B fine-tuned CNN outputs.
#
...
# Candidate B distances are directly comparable.
#
# Pipeline stages:
...
#   - data_out/wind_finetune_pilot_candidateB/{dataSR,dataGT,idx}.npy
#   - Docker image phire-ttk:latest
#   - native python3 with vtk + topologytoolkit (for step 3)
#
# Usage:
#   bash scripts/run_candidateB_topology_pipeline.sh [options]
#
# Options:
...
#
# Outputs:
#   ttk_runs_fixed/topology_finetuning/candidateB_vti/        VTI files
#   ttk_runs_fixed/topology_finetuning/candidateB_topology/   TTK outputs + CSV
# ---------------------------------------------------------------------------
set -euo pipefail
...
WORKDIR="$(pwd)"
DATA_DIR="data_out/wind_finetune_pilot_candidateB"
METHOD="candidateB"
VTI_DIR="ttk_runs_fixed/topology_finetuning/candidateB_vti"
OUT_BASE="ttk_runs_fixed/topology_finetuning/candidateB_topology"

# Baseline outputs — must never be modified.
_PROTECTED_DIRS=(
  "ttk_runs_fixed/cnn"
  "ttk_runs_fixed/gan"
...
# ---------------------------------------------------------------------------
# Safety check: refuse if OUT_BASE would touch protected dirs
# ---------------------------------------------------------------------------
for p in "${_PROTECTED_DIRS[@]}"; do
  if [[ "$(realpath -m "$OUT_BASE")" == "$(realpath -m "$p")"* ]]; then
    echo "[error] OUT_BASE=$OUT_BASE would write inside protected $p. Aborting."
    exit 1
  fi
done
...
for f in "$DATA_DIR/dataGT.npy" "$DATA_DIR/dataSR.npy"; do
  if [[ ! -f "$f" ]]; then
    ...
    echo "        Run scripts/run_physics_finetune_pilot.py first."
    exit 1
  fi
done
...
echo "========================================"
echo "Candidate B topology pipeline"
echo "========================================"
...
python3 scripts/build_candidateB_topology_comparison.py \
  --candidateB-results "$FINAL_DIR/phase_c_results.csv" \
  --baseline-results   "ttk_runs_fixed/combined/phase_c_results.csv" \
  --candidateB-idx     "$DATA_DIR/idx.npy" \
  --out-dir            "$OUT_BASE" \
...
echo "  Comparison   : $OUT_BASE/candidateB_topology_comparison.csv"
echo "  Report       : docs/topology_finetuning_candidateB_topology_eval.md"
echo "========================================"
