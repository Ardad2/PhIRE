...
def import_frozen_canonical_parser(w22: Path):
    """
    Import the exact parser used by the frozen corrected-PD audit.

    IMPORTANT:
    Do not duplicate its finite/global-pair logic here. TTK's global min-max
    persistence pair has finite Birth/Persistence coordinates in these VTUs, so
    merely checking np.isfinite() is insufficient to exclude it.
    """
    canonical_dir = w22.parent / "recompute_pd"
    canonical_py = canonical_dir / "canonical_pd_pilot.py"

    if not canonical_py.is_file():
        raise FileNotFoundError(
            f"Frozen canonical parser not found: {canonical_py}"
        )

    sys.path.insert(0, str(canonical_dir))
    import canonical_pd_pilot as canonical  # type: ignore

    return canonical, canonical_py
...
def load_ttk_pd(artifact: PDArtifact, canonical) -> PD:
    """
    Read the TTK persistence diagram through the exact frozen canonical parser.

    `canonical.read_pd()` returns:
        finite_by_dim, global_pair

    where finite_by_dim[0] and [1] are the audited D0/D1 birth-death arrays and
    the global/nonfinite min-max pair is kept separate.

    We intentionally do NOT reconstruct this logic locally.
    """
    finite, global_pair = canonical.read_pd(artifact.path)
...
def read_frozen_sample_metrics(
    ...
    """
    Read the authoritative corrected PD trio directly from w22_full_sweep.csv.
    """
    run_name = {
        "cnn": "cnn",
        "uv": (
    ...
    for method in METHODS:
        target_run = run_name[method]

        hits = [
            r for r in rows
            if r.get("run") == target_run
            and int(r.get("sample", r.get("sample_idx", "-1"))) == sample
        ]

        if len(hits) != 1:
            raise RuntimeError(
                f"Frozen W22 row lookup method={method} run={target_run}: "
                f"got {len(hits)}"
            )
...
def phase_a(
    ...
    print()
    print("=" * 110)
    print("PHASE A — SAME-DIAGRAM METRIC PARITY + FROZEN REPRODUCTION")
    print("=" * 110)
    ...
    print("PHASE A SAME-DIAGRAM METRIC PARITY + FROZEN REPRODUCTION: PASS")
...
def main() -> int:
    ...
    canonical, canonical_py = import_frozen_canonical_parser(w22)
    ...
    print("frozen canonical parser:", canonical_py)
    print("sample:", sample)
    print("output:", outdir)
    ...
    print("authoritative frozen PD source:", frozen_csv)
    frozen = read_frozen_sample_metrics(frozen_csv, sample)
