#!/usr/bin/env python3
"""
Candidate C-expanded-672: Candidate C topology losses, trained on the
...
Scientific motivation:
  Candidate C uses a critical-value loss (L_crit) that targets GT local-speed
  maxima above an adaptive per-sample threshold (mean + crit_high_z * std),
  acting as a topological-extrema proxy.  Training on 672 samples (4× the
  benchmark size, spanning winter/spring/summer/fall) tests whether the
  topology-aware objective generalises across seasons and whether additional
  training diversity transfers favourably to the 168-sample evaluation set.
  Evaluation on the original 168-sample benchmark makes results directly
  comparable with Candidate C (trained on example_data_fixed/).
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateC_expanded672.log'
...
# Training data: expanded 672-sample seasonal dataset
...
# Outputs
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateC_expanded672'
DATA_OUT  = 'data_out/wind_finetune_candidateC_expanded672'
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate C-expanded-672 fine-tuning')
    print('=' * 64)
    ...
    # Loss configuration is identical to Candidate C1.
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateC_expanded672',
        learning_rate= 1e-5,
        N_epochs     = 3,
        save_every   = 1,
        print_every  = 2,
    ...
    print()
    print('Next steps:')
    print('  Evaluate (compare to CNN baseline and Candidate C):')
    print('    python3 scripts/evaluate_finetune_candidate.py \\')
    print('      --candidate-name candidateC_expanded672 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateC_expanded672 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateC_expanded672_eval')
    print()
    print('  TTK topology pipeline:')
    print('    bash scripts/run_candidate_topology_pipeline.sh \\')
    print('      --method   candidateC_expanded672 \\')
    print('      --data-dir data_out/wind_finetune_candidateC_expanded672')
    print('=' * 64)
