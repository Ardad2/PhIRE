#!/usr/bin/env python3
"""
CandidateE2-fixed-lowlambda-expanded-1344: PyTorch residual refiner with
TTK critical-pair loss, trained on the 1344-sample expanded seasonal
dataset, evaluated on the 168-sample benchmark.

Derived from run_candidateE2_expanded672_ttkcrit_refiner.py (the
L_uv-normalization-fixed base script), with the low-lambda TTK weights from
docs/dataset_generation_and_repair_notes.md Part XIII.4
(LAMBDA_TTKCV=0.004, LAMBDA_TTKPERS=0.002 -- a 10x reduction from the
original 0.04/0.02) and 1344-sample-scale paths substituted throughout.

Loss configuration:
    L_total = L_uv                     (normalized [u, v], matches TF1 B/C)
            + 0.01   * L_speed
            + 0.05   * L_grad
            + 0.001  * L_crit          (Candidate C pool-3 local-maxima proxy)
            + 0.25   * L_levelset      (Candidate B soft level-set at 5/10/15 m/s)
            + 0.004  * L_ttkcv         (MSE at TTK critical vertices; low-lambda)
            + 0.002  * L_ttkpers       (persistence-gap MSE; low-lambda)

TTK constraints must be pre-built for this scale with:
...
Workflow:
  Phase 1 — Training:  train RefinerNet on 1344 expanded CNN SR outputs
            Input:  data_out/wind_mrhr_cnn_expanded1344/  (dataSR.npy, dataGT.npy)
                    generated with scripts/generate_expanded_cnn_sr.py from
                    example_data_topology_expanded_1344/wind_MR-HR.tfrecord
            Constraints: ttk_runs_fixed/topology_finetuning/
                           candidateE2_fixed_lowlambda_expanded1344_constraints/
                           ttk_pd_critical_pairs_gtvalues.npz
            Output: models_fixed/topology_finetuning/
                      wind_finetune_candidateE2_fixed_lowlambda_expanded1344/
  ...
  Phase 2 — Evaluation: apply trained refiner to 168-sample benchmark CNN SR
            ...
            Output: data_out/wind_finetune_candidateE2_fixed_lowlambda_expanded1344/
...
TRAIN_CNN_DIR   = REPO_ROOT / "data_out" / "wind_mrhr_cnn_expanded1344"
...
OUT_DIR  = REPO_ROOT / "data_out" / "wind_finetune_candidateE2_fixed_lowlambda_expanded1344"
LOG_PATH = REPO_ROOT / "logs"    / "wind_finetune_candidateE2_fixed_lowlambda_expanded1344.log"
...
LAMBDA_TTKCV    = 0.004   # low-lambda: 10x lower than the original E2 0.04
LAMBDA_TTKPERS  = 0.002   # low-lambda: 10x lower than the original E2 0.02
...
# Expected pairs-per-sample from the constraint builder (persistence_frac=0.01,
# top_k=64). Every sample hitting exactly top_k=64 is the historically observed
# (and expected) behavior for this scale of dataset -- see preflight check below.
EXPECTED_PAIRS_PER_SAMPLE = 64

# ── Protected output paths ─────────────────────────────────────────────────────
# Broadened relative to the 672 base script per the explicit request to also
# protect the parallel 2688-scale run, the non-expanded E2 variants, and the
# Candidate C/UV expanded1344/2688 results.

_PROTECTED = [
    REPO_ROOT / "data_out_fixed" / "wind_mrhr_cnn",
    REPO_ROOT / "data_out_fixed" / "wind_mrhr_gan",
    REPO_ROOT / "data_out"       / "wind_mrhr_cnn_expanded672",
    REPO_ROOT / "data_out"       / "wind_mrhr_cnn_expanded2688",
    ...
    REPO_ROOT / "data_out"       / "wind_finetune_candidateE2_expanded672",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateE2_fixed",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateE2_fixed_lowlambda",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateE2_fixed_lowlambda_expanded2688",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateC_expanded1344",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateC_expanded2688",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateUV_expanded1344",
    REPO_ROOT / "data_out"       / "wind_finetune_candidateUV_expanded2688",
    ...
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateE2_expanded672",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateE2_fixed",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateE2_fixed_lowlambda",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateE2_fixed_lowlambda_expanded2688",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateC_expanded1344",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateC_expanded2688",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateUV_expanded1344",
    REPO_ROOT / "models_fixed"   / "topology_finetuning" / "wind_finetune_candidateUV_expanded2688",
    REPO_ROOT / "ttk_runs_fixed" / "topology_finetuning" / "candidateE2_expanded672_constraints",
    REPO_ROOT / "ttk_runs_fixed" / "topology_finetuning" / "candidateE2_fixed_constraints",
    REPO_ROOT / "ttk_runs_fixed" / "topology_finetuning" / "candidateE2_fixed_vti",
    REPO_ROOT / "ttk_runs_fixed" / "topology_finetuning" / "candidateE2_fixed_lowlambda_expanded2688_constraints",
    REPO_ROOT / "ttk_runs_fixed" / "topology_finetuning" / "candidateE2_fixed_lowlambda_expanded2688_vti",
    REPO_ROOT / "example_data_fixed",
    REPO_ROOT / "example_data_topology_expanded_672",
    REPO_ROOT / "example_data_topology_expanded_2688",
]
    ...
    def __init__(self, npz_path: Path, patch: int, device: torch.device) -> None:
        ...
        if missing:
            raise ValueError(
                ...
                "Re-run build_candidateE2_expanded672_ttk_constraints.py "
                "with --gt-path/--idx-path/--n-expected for this scale."
            )
...
def _check_preconditions() -> None:
    ...
    gt_shape = np.load(TRAIN_CNN_DIR / "dataGT.npy", mmap_mode="r").shape
    if gt_shape[0] != N_TRAIN:
        sys.exit(
            f"[error] Expanded CNN GT has {gt_shape[0]} samples; expected {N_TRAIN}.\n"
            f"  Regenerate: {TRAIN_CNN_DIR / 'dataGT.npy'}"
        )

    # idx.npy: must exist and have exactly N_TRAIN entries (explicit check,
    # separate from the optional-load in _load_cnn_dir, since silently
    # falling back to positional indices would desync from the constraints
    # NPZ's sample_idx values).
    idx_path = TRAIN_CNN_DIR / "idx.npy"
    if not idx_path.exists():
        sys.exit(
            f"[error] {idx_path} not found. Training requires idx.npy to "
            "align samples with the constraints NPZ's sample_idx."
        )
    idx_train_check = np.load(idx_path)
    if len(idx_train_check) != N_TRAIN:
        sys.exit(
            f"[error] {idx_path} has {len(idx_train_check)} entries; expected {N_TRAIN}."
        )
    if len(set(idx_train_check.tolist())) != N_TRAIN:
        sys.exit(f"[error] {idx_path} contains duplicate sample indices.")
    ...
    eval_idx_path = EVAL_CNN_DIR / "idx.npy"
    if eval_idx_path.exists():
        eval_idx = np.load(eval_idx_path)
        if not np.array_equal(np.sort(eval_idx), np.arange(N_EVAL)):
            sys.exit(
                f"[error] {eval_idx_path} does not contain exactly 0..{N_EVAL - 1}.\n"
                f"  Got range [{int(eval_idx.min())}, {int(eval_idx.max())}], "
                f"{len(eval_idx)} entries."
            )
    ...
    if n_constraints != N_TRAIN:
        sys.exit(
            ...
            "  Re-run build_candidateE2_expanded672_ttk_constraints.py for this scale."
        )

    # sample_count distribution sanity check (expected: every sample hits
    # the top_k=64 cap, matching prior 672-scale behavior). Deviation is not
    # necessarily wrong (e.g. an intentionally different top_k/persistence_frac),
    # but is unexpected and worth surfacing loudly rather than silently
    # training on a differently-shaped constraint set.
    ...
    print(f"[precondition] constraints sample_count: min={sc_min} mean={sc_mean:.2f} max={sc_max}")
    if sc_min != EXPECTED_PAIRS_PER_SAMPLE or sc_max != EXPECTED_PAIRS_PER_SAMPLE:
        print(
            f"  [WARN] sample_count is not uniformly {EXPECTED_PAIRS_PER_SAMPLE} "
            f"(min={sc_min}, max={sc_max}). This is expected only if the "
            "constraint builder was intentionally run with different "
            "--persistence-frac/top_k settings for this scale. If not "
            "intentional, re-check the constraint-building run before training."
        )

    # All training idx values must be present in constraints sample_idx
    idx_train_all = set(int(x) for x in idx_train_check)
    constraint_idx = set(int(x) for x in npz["sample_idx"])
    missing_from_constraints = idx_train_all - constraint_idx
    if missing_from_constraints:
        sys.exit(
            f"[error] {len(missing_from_constraints)} training sample IDs not in "
            f"constraints NPZ (first 10): {sorted(missing_from_constraints)[:10]}\n"
            "  Re-run the constraint builder for this scale."
        )
    print(f"[precondition] All {len(idx_train_all)} training indices confirmed present in constraints.")
...
def main() -> None:
    ...
    print("=" * 64)
    print("CandidateE2-fixed-lowlambda-expanded-1344  TTK Critical-Pair Refiner")
    print("=" * 64)
    ...
    # ── Load training data (1344 expanded) ────────────────────────────────
    ...
    print("\n" + "=" * 64)
    print("CandidateE2-fixed-lowlambda-expanded-1344 complete.")
