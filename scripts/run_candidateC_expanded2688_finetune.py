#!/usr/bin/env python3
"""
Candidate C-expanded-2688: Candidate C topology losses, trained on the
...
Scientific motivation:
  Scaling Candidate C from 1344 to 2688 training samples (four windows per
  season instead of two) tests whether the Candidate C topology-aware proxy
  losses remain robust as the training set doubles again.  All hyperparameters
  and loss weights are identical to candidateC_expanded1344 so the comparison
  is controlled: the only difference is the volume of training data (2688 vs
  1344).  If CandidateC-expanded-2688 looks promising, run
  CandidateUV-expanded-2688 as the matching UV-only ablation.
...
Usage (from repo root, on Spark):
  python3 scripts/run_candidateC_expanded2688_finetune.py \\
    2>&1 | tee logs/wind_finetune_candidateC_expanded2688.log
...
# ---------------------------------------------------------------------------
LOG_PATH = REPO_ROOT / 'logs' / 'wind_finetune_candidateC_expanded2688.log'
...
# Outputs
MODEL_OUT = 'models_fixed/topology_finetuning/wind_finetune_candidateC_expanded2688'
DATA_OUT  = 'data_out/wind_finetune_candidateC_expanded2688'
...
def main() -> None:
    ...
    print('=' * 64)
    print('Candidate C-expanded-2688 fine-tuning')
    print('=' * 64)
    ...
    # Loss configuration is identical to Candidate C1 and candidateC_expanded1344.
    ...
    phire = PhIREGANs(
        data_type    = 'wind_finetune_candidateC_expanded2688',
        learning_rate= 1e-5,
        N_epochs     = 3,
        save_every   = 1,
        print_every  = 2,
    ...
    print()
    print('Next steps:')
    print('  Evaluate (compare to CNN baseline and candidateC_expanded1344):')
    print('    python3 scripts/evaluate_finetune_candidate.py \\')
    print('      --candidate-name candidateC_expanded2688 \\')
    print('      --candidate-dir  data_out/wind_finetune_candidateC_expanded2688 \\')
    ...
    print('      --out-dir        ttk_runs_fixed/topology_finetuning/candidateC_expanded2688_eval')
    print()
    print('  TTK topology pipeline:')
    print('    bash scripts/run_candidate_topology_pipeline.sh \\')
    print('      --method   candidateC_expanded2688 \\')
    print('      --data-dir data_out/wind_finetune_candidateC_expanded2688')
    print()
    print('  If results look promising, run CandidateUV-expanded-2688 as the')
    print('  matching UV-only ablation to isolate the effect of the C losses.')
    print('=' * 64)
