...
## XXXVI.120 Production MT-distance call site — defaults confirmed as authoritative

The project/audit call-site inspection established an important fact:
...
No merge-tree algorithmic parameter was explicitly overridden by the original
distance script.

Therefore the audited historical MT values were produced from the local TTK
1.3.0 runtime defaults, subject to the VTK wrapper's normal `Backend` logic.

The audit already recorded the following runtime defaults:

```text
AssignmentSolver       = 0
BranchDecomposition    = 1
DistanceSquaredRoot    = 1
Epsilon1UseFarthestSaddle = False
KeepSubtree            = 0
NormalizedWasserstein  = 1
```

This is consistent with the `Backend == 0` path observed in
`ttkMergeTreeDistanceMatrix::run()`, which forces:
...
unless field-data parameters are being used.

### Consequence for matching extraction

The matching extractor must reproduce the exact local runtime/default
configuration rather than substitute parameter choices from current TTK
documentation or from the threshold-3 qualitative visualization pipeline.

The threshold-3 tree remains display-only and is not automatically the same
tree representation used internally by the numerical distance.

---

## XXXVI.121 One final narrow preflight before implementing the extractor

The latest pasted output was sufficient to confirm the production call site and
several runtime defaults, but the large source dump did not preserve the
specific body of `convertBranchDecompositionMatching()` or the complete runtime
default list.

Before writing the C++ matching extractor, perform a narrow preflight to capture:

```text
1. the complete runtime getter dump;
2. exact location/body of convertBranchDecompositionMatching();
3. exact location/body of constructTrees();
4. ttkMergeTreeDistanceMatrix class inheritance and protected/public boundary;
5. the VTK wrapper defaults for Backend and UseFieldDataParameters.
```

This will determine whether the cleanest extractor should:

```text
A. subclass ttkMergeTreeDistanceMatrix and reuse constructTrees(), or
B. construct ftm::MergeTree objects directly.
```

The goal remains:

```text
direct extracted distance
    ==
frozen audited sample-69 MT distance
```

before any matching is interpreted or visualized.

---

## XXXVI.122 Matching-extractor implementation path — simplified

The latest source inspection resolved an important implementation question.

`ttkMergeTreeDistanceMatrix` inherits from the base distance-matrix module via
protected inheritance, but the VTK-to-internal-tree conversion itself is not a
private member of that wrapper.

Instead, TTK provides the namespace utility:

```cpp
ttk::ftm::constructTrees(...)
```

in:

```text
/usr/local/include/ttk/vtk/ttkMergeTreeUtils.h
```

For ordinary merge-tree multiblocks containing at least two blocks, this helper
extracts:

```text
block 0 -> nodes.vtu
block 1 -> arcs.vtu
block 2 -> segmentation, if present
```

and constructs the internal tree with:

```cpp
makeTree<dataType>(treesNodes[i], treesArcs[i])
```

Therefore a matching extractor does **not** need to subclass
`ttkMergeTreeDistanceMatrix` merely to reproduce its VTK-to-tree conversion.

### Production-default confirmation

The original project distance code instantiated:

```python
flt = ttk.ttkMergeTreeDistanceMatrix()
```

and did not override merge-tree algorithmic settings.

The audit's runtime getter dump confirms key defaults:
...
The user has separately audited the complete default configuration and can
supply that audit if needed.

Accordingly, matching extraction should reproduce the frozen audited default
TTK configuration rather than perform a new parameter-selection exercise.

### Remaining implementation prerequisites

Only two concrete items remain before writing/running the first extractor:

```text
1. inspect the actual MergeTreeBase::convertBranchDecompositionMatching()
   implementation, rather than the clustering call site accidentally selected
   by the previous grep;

2. identify the exact original sample-69 GT/SR merge-tree port files used by
   the audited numerical distance for CNN, UV, and F1.
```

The first extractor should initially target the **raw pre-postprocessing**
matching and require exact numerical reproduction of the frozen audited MT
distance before any node correspondence is interpreted.

Only after this reproduction passes should a postprocessed/converted matching
be mapped onto the qualitative abstract tree.

---

## XXXVI.123 Branch-decomposition matching conversion semantics — confirmed

The actual `MergeTreeBase::convertBranchDecompositionMatching()` implementation
was recovered.

For every raw branch-decomposition matching tuple:

```text
(node1, node2, cost)
```

TTK retrieves each matched node's `origin`, determines the higher and lower
endpoint of the corresponding branch by tree level, and emits up to two
converted node correspondences:

```text
tree1 higher endpoint <-> tree2 higher endpoint
tree1 lower endpoint  <-> tree2 lower endpoint
```

while skipping the full-merge root case and isolated nodes.

The original tuple `cost` is copied to each emitted converted endpoint tuple.

Consequences:

```text
1. the postprocessed matching is a node-endpoint representation derived from
   branch-level matches;

2. one raw branch match can become two node-to-node tuples;

3. converted tuple costs must NOT be summed to reconstruct the distance,
   because a branch cost can be duplicated across the two endpoint tuples;

4. the converted matching is appropriate for correspondence visualization,
   not as an additive distance decomposition.
```

This is consistent with the earlier observation that insertion/deletion dummy
assignments are not emitted as ordinary real-to-real tuples.

---

## XXXVI.124 Exact sample-69 audited MT source artifacts identified

The original non-display merge-tree port files needed for direct matching
reproduction are present.

Candidate F1 (`candidateF_grad_E2_low_expanded2688_topology`):

```text
GT:
    .../mt/GT/candidateF_grad_E2_low_expanded2688_GT_s69_speed_p160_x0_y0_mt_port_0.vtu
    .../mt/GT/candidateF_grad_E2_low_expanded2688_GT_s69_speed_p160_x0_y0_mt_port_1.vtu
    .../mt/GT/candidateF_grad_E2_low_expanded2688_GT_s69_speed_p160_x0_y0_mt_port_2.vti

SR:
    .../mt/SR/candidateF_grad_E2_low_expanded2688_SR_s69_speed_p160_x0_y0_mt_port_0.vtu
    .../mt/SR/candidateF_grad_E2_low_expanded2688_SR_s69_speed_p160_x0_y0_mt_port_1.vtu
    .../mt/SR/candidateF_grad_E2_low_expanded2688_SR_s69_speed_p160_x0_y0_mt_port_2.vti
```

Matched UV control (`candidateUV_expanded2688_topology`):

```text
GT:
    .../mt/GT/candidateUV_expanded2688_GT_s69_speed_p160_x0_y0_mt_port_0.vtu
    .../mt/GT/candidateUV_expanded2688_GT_s69_speed_p160_x0_y0_mt_port_1.vtu
    .../mt/GT/candidateUV_expanded2688_GT_s69_speed_p160_x0_y0_mt_port_2.vti

SR:
    .../mt/SR/candidateUV_expanded2688_SR_s69_speed_p160_x0_y0_mt_port_0.vtu
    .../mt/SR/candidateUV_expanded2688_SR_s69_speed_p160_x0_y0_mt_port_1.vtu
    .../mt/SR/candidateUV_expanded2688_SR_s69_speed_p160_x0_y0_mt_port_2.vti
```

The CNN baseline also has sample-69 original ports under:

```text
ttk_runs_fixed/cnn/mt/
```

with GT/SR port 0, 1, and 2 files.

These artifacts are distinct from the newly generated threshold-3 qualitative
trees and should be used to reproduce the audited numerical MT distances.

---

## XXXVI.125 Matching extractor — remaining build-environment check

The scientific/API prerequisites are now sufficient for implementation.

Before committing the C++ extractor, inspect the installed TTK/VTK CMake and
library targets so the extractor can be built against the exact audited
container without guessing link flags.

The build preflight should record:

```text
/usr/local/lib/cmake TTK/VTK package locations
installed libttk*.so names
vtk-config / pkg-config availability
CMake package target information if available
```

After this one build-environment check, the extractor can be written to:

```text
read original GT/SR port files
construct internal trees with ttk::ftm::constructTrees()
compute raw matching with postprocess=false
compute converted matching with postprocess=true
assert both distances reproduce the frozen audited MT distance
export raw and converted matching CSVs
```

No further algorithmic-parameter audit is required unless a numerical
reproduction mismatch occurs.

---

## XXXVI.126 Matching-extractor build environment — READY

The audited `phire-ttk` container exposes both installed libraries and CMake
package targets needed for a standalone matching extractor.

Relevant installed shared libraries include:

```text
/usr/local/lib/libttkBasemergeTreeClustering.so
/usr/local/lib/libttkBasemergeTreeDistanceMatrix.so
/usr/local/lib/libttkMergeTree.so
/usr/local/lib/libttkMergeTreeDistanceMatrix.so
```
...
The exported VTK target exists:

```text
ttk::vtk::ttkMergeTreeDistanceMatrix
```
...
Therefore the first standalone extractor can be built inside `phire-ttk` with
CMake using `find_package(TTKVTK CONFIG REQUIRED)` and linking against the
exported merge-tree-distance-matrix target.

### First extractor acceptance policy

The extractor will use the original numerical-distance MT port files, not the
threshold-3 display trees.

For sample 69 it will run:
...
Fresh internal trees must be reconstructed for every run because
`MergeTreeDistance::execute()` preprocesses/mutates its tree arguments.

Required acceptance checks:

```text
distance_raw == frozen audited mt_distance
distance_converted == frozen audited mt_distance
distance_raw == distance_converted
```

within a tight numerical tolerance.

No matching tuple will be interpreted until all three pairs satisfy those
distance-reproduction checks.

### Matching semantics retained

Raw matching:

```text
branch-decomposition node/branch matches produced directly by the distance DP
```

Converted matching:

```text
endpoint correspondences generated by
convertBranchDecompositionMatching()
```

One raw branch match can yield two converted endpoint tuples carrying the same
branch cost. Consequently converted matching costs are non-additive and are for
correspondence visualization only.

Insertion/deletion dummy assignments are not ordinary emitted real-to-real
matching tuples.

---

## XXXVI.127 Numerical MT environment correction — host VTK 9.6 is authoritative

The environment split for the topology pipeline was clarified:

```text
VTI -> PD/MT extraction:
    phire-ttk Docker
    VTK 9.1
    TTK command-line extraction tools

PD/MT -> numerical distances:
    HOST /usr/bin/python3
    VTK 9.6.0
    host topologytoolkit bindings
```

This means the exact matching corresponding to the audited numerical MT
distance must be recovered in the **host numerical-distance environment**, not
inside `phire-ttk`.

The host was verified as:
...
VTK:
    9.6.0
    module under /usr/local/lib/python3.12/site-packages

TTK Python:
    /usr/local/lib/python3.12/site-packages/topologytoolkit
```

The original sample-69 CNN MT port files load correctly under host VTK 9.6.

The frozen corrected-PD/MT joined table gives for sample 69 CNN:
...
Host `libttkMergeTreeDistanceMatrix.so` is linked against VTK 9.6 libraries
under `/usr/local/lib`.

Therefore the matching extractor should compile and execute directly on the
host against:

```text
VTK_DIR=/usr/local/lib/cmake/vtk-9.6
TTKBase_DIR=/usr/local/lib/cmake/ttkBase
```

while avoiding the Docker VTK 9.1 environment entirely for distance matching.

---

## XXXVI.128 Host-9.6 matching extractor prepared

A host-specific extractor bundle was prepared with:

```text
extract_ttk_mt_matching.cpp
CMakeLists.txt
run_sample69_mt_matching_extract_host96.sh
```

The C++ extractor itself is unchanged from the prior version.

The host runner:

```text
1. prints the host Python / VTK / TTK environment;
2. builds against host VTK 9.6 and host TTKBase;
3. verifies executable linkage;
4. computes raw and converted matchings for:
       GT <-> CNN
       GT <-> matched UV
       GT <-> Candidate F1
5. reads the frozen sample-69 MT values from corrected_pd_mt_joined.csv;
6. tolerates the table's `MT` column name;
7. resolves UV/F1 rows defensively from exact/fuzzy audited identifiers;
8. requires raw and converted distances to reproduce the frozen value within
   1e-6;
9. refuses interpretation if any reproduction check fails.
```
...
No matching tuple should be interpreted until the script reports:

```text
ALL THREE SAMPLE-69 MT DISTANCES REPRODUCED: PASS
```

---

## XXXVI.129 Sample-69 exact TTK MT matching reproduction — PASS

The host-VTK-9.6 matching extractor compiled successfully and reproduced the
frozen audited merge-tree distances exactly for all three focal comparisons.
...
VTK = 9.6.0
TTK Python = /usr/local/lib/python3.12/site-packages/topologytoolkit
C++ extractor linked to /usr/local VTK 9.6 and host TTK
```
...
```text
CNN:
    frozen MT     = 5.17050838470459
    raw MT        = 5.17050838470459
    converted MT  = 5.17050838470459
    all diffs     = 0

UV:
    frozen MT     = 5.388279914855957
    raw MT        = 5.388279914855957
    converted MT  = 5.388279914855957
    all diffs     = 0

Candidate F1:
    frozen MT     = 5.357361793518066
    raw MT        = 5.357361793518066
    converted MT  = 5.357361793518066
    all diffs     = 0
```
...
Thus every focal raw matching tuple became exactly two converted endpoint
tuples in these runs.

The observed sample-69 ranking is:
...
```text
F1 vs CNN:
    +3.6138% MT distance (worse)

F1 vs UV:
    -0.5738% MT distance (slightly better)
```

This exactly matches the previously audited sample-69 PD+/MT- discordance
interpretation.

### Scientific status

The matching-recovery path is now numerically validated.

The exported raw and converted matching tuples are no longer hypothetical or
heuristic: they come from the same host TTK/VTK numerical-distance environment
and reproduce the frozen audited MT values exactly.

However, converted matching costs are still non-additive because one branch
match can become two endpoint correspondences carrying the same branch cost.

The next required stage is to audit the converted node IDs against the original
numerical-tree node VTUs before mapping anything onto the threshold-3
qualitative abstract tree.

---

## XXXVI.130 Matching-ID audit script prepared

Prepared:

```text
audit_sample69_ttk_matching_ids.py
```

Purpose:

```text
1. read the converted matching CSVs;
2. read the ORIGINAL numerical-tree port-0 VTUs with host VTK 9.6;
3. inventory NodeId / VertexId / Scalar / CriticalType arrays;
4. determine whether matching IDs correspond to NodeId values, point-row
   indices, or both;
5. verify every converted matching ID resolves;
6. enrich each converted tuple with original numerical-tree metadata;
7. report unique matched nodes and duplicate endpoint usage;
8. deliberately avoid threshold-3 mapping at this stage.
```
...
```text
cnn_converted_matching_enriched.csv
uv_converted_matching_enriched.csv
f1_converted_matching_enriched.csv
sample69_ttk_matching_id_audit.csv
sample69_ttk_matching_id_audit_summary.txt
```

Only after this ID audit passes should a numerical-tree-to-authoritative-C-order
threshold-3 bridge be constructed.

---

## XXXVI.131 Converted TTK matching-ID semantics — VALIDATED

The converted sample-69 matching tuples were audited against the ORIGINAL
numerical-tree `mt_port_0.vtu` node grids using the host VTK 9.6 environment.

For all three focal comparisons:
...
NodeId == VTK point-row index:
    true
```

Therefore the converted matching IDs are unambiguous with respect to the
original numerical MT node grids:

```text
matching tuple node id == NodeId == point-row index
```
...
CNN:
    raw matches                = 312
    converted matches          = 624
    converted == 2 * raw       = true
    unique converted pairs     = 624
    unique GT endpoint IDs     = 619 / 2193
    unique SR endpoint IDs     = 623 / 1144

UV:
    raw matches                = 252
    converted matches          = 504
    converted == 2 * raw       = true
    unique converted pairs     = 504
    unique GT endpoint IDs     = 502 / 2193
    unique SR endpoint IDs     = 504 / 808

Candidate F1:
    raw matches                = 309
    converted matches          = 618
    converted == 2 * raw       = true
    unique converted pairs     = 618
    unique GT endpoint IDs     = 613 / 2195
    unique SR endpoint IDs     = 618 / 1765
```

Every converted pair is unique, but some individual endpoints are reused across
different branch-endpoint correspondences. This is expected for adjacent
matched branches sharing a saddle/endpoint and means the converted matching is
not a one-to-one node bijection.

The exact factor-of-two conversion also shows that, for these three sample-69
runs, every emitted raw branch match produced both endpoint correspondences;
none was lost to the full-merge-root or isolated-node filtering branches in
`convertBranchDecompositionMatching()`.

### Interpretation boundary

The ratios of unique matched endpoint IDs to total numerical-tree nodes should
NOT be interpreted as "percent of topology matched." The underlying edit /
assignment formulation contains insertion/deletion costs that are not exported
as ordinary real-to-real tuples, and converted endpoint reuse is possible.

The validated statement is only:

> The explicit real-to-real TTK correspondences can now be resolved
> unambiguously to original numerical-tree nodes and enriched with their
> NodeId, VertexId, Scalar, CriticalType, and spatial coordinates.

---

## XXXVI.132 Next bridge: numerical threshold-0 tree -> authoritative threshold-3 tree

The next stage must bridge two deliberately distinct representations:

```text
audited numerical MT:
    original saved MT ports
    persistence threshold = 0
    historical numerical source orientation/provenance

qualitative abstract MT:
    regenerated authoritative repaired C-order track
    frozen display persistence threshold = 3
```

The bridge must therefore be validated through `VertexId` / critical-vertex
identity rather than NodeId equality.

Because the historical numerical writer used the legacy F-order/transposed
track while the new qualitative trees use authoritative C-order, the bridge
must explicitly test both:

```text
identity VertexId mapping

and

legacy transpose mapping:
    old_x = old_VertexId % 160
    old_y = old_VertexId // 160
...
No correspondence lines should be drawn on the threshold-3 abstract trees until
the overlap test determines which mapping is actually supported by the saved
artifacts.

A particularly strong validation is:

```text
For each GT/CNN/UV/F1 threshold-3 node:
    does its authoritative VertexId occur among the original numerical-tree
    vertices under identity mapping?

    does it occur under transpose mapping?
```

The supported orientation should produce a dominant, ideally exact, overlap.

After that orientation bridge is frozen, the explicit TTK matching endpoints
that survive into the threshold-3 display tree can be projected onto the
validated abstract hierarchy.

---

## XXXVI.133 Numerical -> threshold-3 Vertex bridge result — mixed provenance confirmed

The sample-69 bridge preflight tested both identity and legacy-transpose
VertexId mappings between the ORIGINAL numerical threshold-0 merge trees and
the regenerated authoritative C-order threshold-3 logical trees.
...
CNN GT -> t3 GT:
    identity   3/58   = 5.17%
    transpose 57/58   = 98.28%

CNN SR -> t3 CNN:
    identity   2/12   = 16.67%
    transpose 10/12   = 83.33%
...
UV SR -> t3 UV:
    identity   2/18   = 11.11%
    transpose 16/18   = 88.89%

F1 GT -> t3 GT:
    identity  57/58   = 98.28%
    transpose  3/58   = 5.17%

F1 SR -> t3 F1:
    identity  35/38   = 92.11%
    transpose  4/38   = 10.53%
```

Therefore there is no single global orientation bridge:

```text
CNN / UV numerical trees:
    strongly support legacy transpose -> authoritative C-order mapping

F1 numerical trees:
    strongly support identity -> authoritative C-order mapping
```

This is direct artifact evidence of mixed numerical-tree provenance/orientation
across method families.

It also explains why NodeId equality across the numerical and new qualitative
tracks must never be assumed.

### Exact converted matching survival onto t3

The exact TTK converted endpoint correspondences almost never survive as exact
node pairs on both threshold-3 display trees:
...
Therefore the threshold-3 abstract hierarchy is **not an appropriate direct
canvas for the exact numerical TTK matching**.

This is not a failure of the matching audit. It reflects the intentional
difference between:

```text
numerical metric representation:
    threshold 0, thousands of nodes

qualitative display representation:
    threshold 3, 12-58 nodes
```

and the fact that TTK matching endpoints are branch-decomposition endpoints
from the numerical tree, not guaranteed to remain explicit critical nodes after
the independent threshold-3 simplification.

### Consequence

Do NOT project the exact numerical matching onto the threshold-3 abstract trees
by nearest node, nearest scalar, nearest spatial coordinate, or ancestor
heuristics. Such a figure would no longer represent the exact audited TTK
matching.

The threshold-3 figure remains useful as a separate qualitative hierarchy view.

---

## XXXVI.134 New mandatory check: cross-provenance GT equivalence

Because CNN/UV and F1 numerical trees have different orientation/provenance and
even slightly different raw GT node counts:

```text
CNN/UV GT: 2193 nodes
F1 GT:     2195 nodes
```

a cross-GT numerical equivalence check is required before using the small
sample-69 CNN-vs-F1 MT difference as explanatory evidence.

Run the already validated host extractor on:
...
using the original numerical MT node/arc ports.

Key question:

```text
Are the method-specific GT trees distance-zero equivalent under the same
audited MergeTreeDistance?
```

If all cross-GT distances are zero, the raw node-count/orientation differences
collapse to the same numerical merge-tree representation and cross-method MT
comparison remains strongly supported.

If CNN/UV <-> F1 GT distance is nonzero, quantify it before interpreting the
small sample-69 difference:

```text
CNN MT = 5.17050838470459
F1 MT  = 5.357361793518066
gap    = 0.1868534088134766
```

A non-negligible GT-reference discrepancy could contribute to that gap and
must be separated from SR hierarchy differences.

This check is now higher priority than producing an exact-matching figure.

---

## XXXVI.135 Cross-provenance GT merge-tree equivalence — PASS

The validated host-VTK-9.6 / host-TTK matching extractor was run directly on
the three method-specific sample-69 GT numerical merge trees:
...
All three pairwise distances were exactly zero:

```text
CNN GT <-> UV GT:
    distance = 0
    raw matching count = 1096
...
but these representation/provenance differences collapse to exactly the same
merge-tree object under the audited TTK `MergeTreeDistance`.

### Consequence for sample-69 interpretation

The previously observed sample-69 distances:

```text
CNN = 5.17050838470459
F1  = 5.357361793518066
UV  = 5.388279914855957
```

can therefore be compared against a numerically equivalent GT reference.

The CNN-vs-F1 gap:

```text
5.357361793518066 - 5.17050838470459
= 0.1868534088134766
```

is not explained by the method-specific GT orientation/provenance difference.

Thus the sample-69 statement is strengthened:

> Candidate F1 has a slightly larger audited merge-tree distance to GT than
> CNN for this sample, even though its frozen threshold-3 qualitative hierarchy
> contains substantially more surviving structure. The discrepancy reflects
> the SR-side merge-tree comparison, not a mismatch between the GT references.

Likewise F1 remains slightly better than the matched UV control in MT distance:

```text
UV - F1 = 0.030918121337890625
```

approximately 0.57% of the UV distance.

### Remaining interpretation boundary

The threshold-3 abstract trees and the exact numerical TTK matching must remain
separate representations:

```text
threshold-3 abstract tree:
    qualitative surviving hierarchy

threshold-0 numerical matching:
    exact audited TTK correspondence / distance representation
```

The exact converted matching almost never survives directly onto the
threshold-3 display nodes, so no heuristic projection should be presented as an
exact TTK matching.

The next explanatory target should operate directly on the validated numerical
threshold-0 matching, ideally by decomposing:

```text
real-to-real branch relabeling
vs.
delete/insert (unmatched branch) contributions
```

under the same TTK objective.

---

## XXXVI.136 Next target: exact numerical MT cost decomposition

After validating exact host-TTK matching recovery and proving cross-provenance
GT distance-zero equivalence, the next target is to explain the sample-69
numerical MT score itself.
...
real-to-real branch relabeling
vs.
delete/insert contributions
```

for CNN, matched UV, and Candidate F1.

This decomposition must be derived from the exact local TTK 1.3.0 source rather
than inferred from unmatched node counts.

### Why unmatched-node counting is insufficient

The exported raw matching contains real-to-real branch matches only. TTK's
assignment also contains dummy row/column choices representing deletion and
insertion. Those costs can involve branch/subtree structure and are not
necessarily recoverable by simply taking all nodes absent from `outputMatching`.

Therefore the exact implementations of:

```text
relabelCost / relabelCostOnly
deleteCost
insertCost
assignment cost-matrix construction
treeBackTable / forestBackTable backtracking
final distance normalization / square-root path
```

must be inspected before producing a quantitative cost split.

### Prepared preflight tools

Two scripts were prepared:

```text
inspect_ttk_mt_cost_decomposition_source.sh
summarize_sample69_raw_matching_costs.py
```

The first extracts the relevant exact local TTK source sections.

The second reports descriptive aggregates of the already validated raw
real-to-real matching costs:

```text
sum(relabel_cost)
sum(relabel_cost^2)
sqrt(sum(relabel_cost))
sqrt(sum(relabel_cost^2))
min / mean / max
```

These aggregates are explicitly non-interpretive until the TTK source audit
establishes how the tuple `relabel_cost` relates to the final squared/rooted
objective.

No numerical cost-decomposition claim should be made before that source audit.

---

## XXXVI.137 Exact aggregate MT cost split is now justified

The local TTK source establishes the following for the audited configuration.

### Cost convention

`wassersteinPower_ = 2`, `normalizedWasserstein_ = true`, and
`distanceSquaredRoot_ = true`.

`deleteCost()` computes squared Euclidean birth/death distance to the diagonal
projection under the normalized birth/death coordinates. `insertCost()` is
identical to `deleteCost()`.

`relabelCostOnly()` computes squared Euclidean distance between normalized
birth/death pairs.

Because `keepSubtree_ = false`, `relabelCost()` does not replace a real-real
relabel with delete+insert; the exported raw tuple cost is the pure relabel cost.

### Assignment / backtracking semantics

The forest assignment cost matrix contains:

```text
real-real:
    treeTable[child1][child2]
...
The assignment solver returns real and dummy assignments, and
`postprocessAssignment()` sums the cost of **all** of them.

However, only assignments whose two indices are real are copied into
`forestBackTable`.

`computeMatching()` then backtracks the tree/forest tables and exports only
explicit real-real matched nodes, storing `relabelCost()` for each one.

Therefore dummy delete/insert choices are included in the DP objective but are
not emitted into `outputMatching`.

### Exact aggregate identity

For the audited default setup:
...
there is no post-DP min/max reweighting.

Hence, if:

```text
d = final TTK MT distance

R = sum of relabel_cost over the validated RAW real-real outputMatching
```

then the exact aggregate nonmatching contribution is:
...
where `N` is the total delete/insert contribution in the squared objective.

This does **not** yet separate tree1 deletion from tree2 insertion and does not
identify individual dummy assignments. That requires further instrumentation.

Prepared:

```text
decompose_sample69_ttk_mt_costs.py
```

It reports for CNN / UV / F1:

```text
distance
distance^2
real-real match count
R = relabel squared contribution
N = delete/insert squared residual
R / d^2
N / d^2
sqrt(R), sqrt(N) as component magnitudes
```

and decomposes pairwise squared-objective differences:
...
into relabel-delta plus nonmatching-delta exactly.

The square-root component magnitudes are diagnostic only; they combine by
quadrature, not addition.

---

## XXXVI.138 Sample-69 aggregate MT cost decomposition — COMPLETE

The exact aggregate decomposition was run on the validated raw TTK matching.

Results:

```text
CNN:
    d^2 = 26.734156956300467
    real-real relabel contribution = 12.513538950755901
    delete/insert contribution     = 14.220618005544566
    relabel share                  = 46.8073%
    nonmatching share              = 53.1927%

UV:
    d^2 = 29.03356044084012
    real-real relabel contribution = 11.73688746982566
    delete/insert contribution     = 17.296672971014459
    relabel share                  = 40.4252%
    nonmatching share              = 59.5748%

F1:
    d^2 = 28.701325386647113
    real-real relabel contribution = 8.6506304465471544
    delete/insert contribution     = 20.050694940099959
    relabel share                  = 30.1402%
    nonmatching share              = 69.8598%
```

Pairwise squared-objective decomposition:

```text
F1 - CNN:
    total      +1.9671684303466463
    relabel    -3.8629085042087468
    nonmatch   +5.8300769345553931

F1 - UV:
    total      -0.33223505419300636
    relabel    -3.086257023278506
    nonmatch   +2.7540219690854997

UV - CNN:
    total      +2.2994034845396527
    relabel    -0.77665148093024072
    nonmatch   +3.0760549654698934
```

Interpretation:

> F1 does not score worse than CNN because its explicitly matched branches are
> more expensive to relabel. The opposite is true: its matched-branch relabel
> contribution is substantially lower. F1's slightly worse total MT distance is
> driven by a larger nonmatching delete/insert contribution, which more than
> offsets the relabel improvement.

Likewise, F1 is slightly better than UV overall because its lower relabel
contribution outweighs its larger nonmatching contribution.

This is the clearest numerical explanation obtained so far for the sample-69
PD+/MT discordance.

---

## XXXVI.139 Next validation: split nonmatching cost into deletions vs insertions

A stronger nodewise attribution can be attempted without patching TTK internals
by reproducing the exact preprocessing explicitly, retaining the internal
branch-decomposition trees, then:

```text
1. run MergeTreeDistance with preprocess=false on those already-preprocessed
   trees;
2. collect the raw real-real matched branch-node IDs;
3. mark every remaining active tree1 branch as a deletion candidate;
4. mark every remaining active tree2 branch as an insertion candidate;
5. sum TTK's own deleteCost() / insertCost() over those branches;
6. require:

   d^2
   =
   sum(real-real relabel costs)
   + sum(tree1 delete costs)
   + sum(tree2 insert costs)

   within tight float-level tolerance.
```

This is a validation, not an assumption.

If exact recomposition passes, then deletion-vs-insertion attribution and the
largest individual unmatched branches are allowed.

If it fails, nodewise unmatched attribution is rejected and direct dummy
assignment instrumentation is required instead.

Prepared bundle:

```text
mt_cost_attribution_bundle.zip
```

containing:

```text
extract_ttk_mt_cost_attribution.cpp
CMakeLists.txt
run_sample69_mt_cost_attribution.sh
```

---

## XXXVI.140 Nodewise deletion / insertion attribution — PASS

The explicit-preprocessing host-VTK-9.6 / host-TTK nodewise attribution
experiment passed exact-objective recomposition for CNN, UV, and Candidate F1.
...
These are negligible relative to total squared objectives (~26.7-29.0), so the
nodewise split is validated.

### Exact squared-objective components

```text
CNN:
    relabel   = 12.5135389508
    deletion  = 13.4329899472
    insertion =  0.7876258834
    total     = 26.7341569563

UV:
    relabel   = 11.7368874698
    deletion  = 16.7259952466
    insertion =  0.5706744490
    total     = 29.0335604408

F1:
    relabel   =  8.6506304465
    deletion  = 12.7921976921
    insertion =  7.2585014974
    total     = 28.7013253866
```
...
Within the nonmatching term only:
...
### Active branch-decomposition nodes after audited preprocessing

```text
GT active nodes (all runs): 1096

CNN SR:
    active   573
    matched  312
    unmatched / inserted 261

UV SR:
    active   405
    matched  252
    unmatched / inserted 153

F1 SR:
    active   885
    matched  309
    unmatched / inserted 576
```

GT-side unmatched / deleted nodes:
...
### F1 vs CNN — decisive decomposition

```text
delta relabel   = -3.8629085042
delta deletion  = -0.6407922551
delta insertion = +6.4708756140
--------------------------------
delta total     = +1.9671748547
```

Therefore F1's slightly worse sample-69 MT distance than CNN is not caused by
either matched-branch relabeling or GT-side deletion. Both are lower for F1.

The entire deterioration is driven by the much larger SR-side insertion term.

Preferred interpretation:

> Candidate F1 reduces both the cost of relabeling explicitly matched branches
> and the aggregate cost of GT-side branch deletions relative to CNN. Its
> slightly larger total merge-tree distance is instead caused by a much larger
> SR-side insertion penalty: F1 contains substantially more active
> branch-decomposition structure, but much of that additional structure is not
> paired with GT by the optimal TTK edit matching.

### F1 vs UV

```text
delta relabel   = -3.0862570233
delta deletion  = -3.9337975544
delta insertion = +6.6878270484
--------------------------------
delta total     = -0.3322275293
```

F1 is slightly better overall than UV because its improvements in matched
relabeling plus GT-side deletion (~7.02 squared-cost units) narrowly outweigh
its ~6.69 increase in insertion cost.

### UV vs CNN

```text
delta relabel   = -0.7766514809
delta deletion  = +3.2930052993
delta insertion = -0.2169514344
--------------------------------
delta total     = +2.2994023840
```

UV's worse MT score relative to CNN is driven primarily by increased GT-side
deletion cost.

### Highest-cost unmatched branches

CNN's and UV's highest-cost unmatched branches are overwhelmingly GT-side
deletions.

F1 is qualitatively different: its top three unmatched costs are all SR-side
insertions, and 6 of its top 12 unmatched costs are insertions.

This supports the aggregate decomposition without replacing it.

### Important boundary

The raw `persistence_raw` column is not itself the TTK cost. TTK computes
delete/insert costs from normalized birth/death coordinates under the audited
normalized-Wasserstein configuration. Do not rank or interpret unmatched
branches from raw persistence alone.

---

## XXXVI.141 Next validation before spatial localization

The explicit-preprocessing run should next be compared directly against the
already validated production-style raw matching CSVs:

```text
cnn_raw_matching.csv
uv_raw_matching.csv
f1_raw_matching.csv
```
...
```text
cost_attribution/cnn_raw_matching_manual_preprocess.csv
cost_attribution/uv_raw_matching_manual_preprocess.csv
cost_attribution/f1_raw_matching_manual_preprocess.csv
```
...
same (tree1_node_id, tree2_node_id) pair set
same relabel cost for each pair within float tolerance
```

This freezes that the nodewise attribution used the exact same optimal raw
matching as the production-style extractor, rather than merely another
equal-cost optimum.

Only after this pair-level equivalence check should the highest-cost unmatched
branches be mapped back to original `VertexId` / spatial locations for
visualization.

---

## XXXVI.142 Manual-preprocess raw matching == production-style raw matching — EXACT PASS

The explicit-preprocessing nodewise attribution run was compared pair-by-pair
against the already validated production-style raw TTK matching.
...
CNN:
    production rows       = 312
    manual rows           = 312
    production-only pairs = 0
    manual-only pairs     = 0
    max relabel cost diff = 0
...
Thus the nodewise delete/insert attribution is tied to the exact same raw
real-to-real optimum as the production-style extractor, not merely to an
equal-cost alternative matching.

Status:

```text
ALL THREE RAW MATCHINGS IDENTICAL: PASS
```

This closes the matching-equivalence validation.

---

## XXXVI.143 Spatial localization of unmatched branches — prepared

The next stage is to map the now-validated unmatched branch-decomposition nodes
back to the original numerical-tree `NodeId` / `VertexId`, then into the
authoritative repaired C-order 160x160 spatial frame.

Frozen orientation bridge:

```text
CNN numerical GT/SR -> transpose -> authoritative C-order
UV  numerical GT/SR -> transpose -> authoritative C-order
F1  numerical GT/SR -> identity  -> authoritative C-order
```

Prepared:

```text
map_sample69_unmatched_branches_spatial.py
```

The script:

```text
1. reads the validated cost_attribution/{cnn,uv,f1}_unmatched_branches.csv;
2. reads the corresponding ORIGINAL numerical-tree port-0 VTUs;
3. requires every unmatched node_id and origin_id to resolve exactly as
   NodeId == VTK point-row index;
4. reads the original numerical VertexId;
5. applies the already frozen method-family orientation bridge;
6. writes authoritative C-order endpoint coordinates for both branch endpoints;
7. ranks unmatched branches by exact TTK nonmatching cost;
8. writes per-method enriched spatial CSVs and a compact top-branch summary.
```

No nearest-neighbor or heuristic mapping is used.

Only after the NodeId/VertexId mapping passes should a spatial overlay figure be
rendered.

---

## XXXVI.144 Unmatched-branch -> authoritative C-order spatial mapping — PASS

The validated unmatched branch-decomposition nodes were mapped back to the
original numerical-tree NodeId / VertexId and then into the authoritative
C-order 160x160 spatial frame.

Every unmatched node and every paired origin resolved exactly against the
original numerical-tree port-0 VTU:
...
Frozen orientation bridge used:
...
No nearest-neighbor or heuristic mapping was used.

### Highest-cost spatially localized unmatched branches

CNN top unmatched costs are all GT-side deletions. Examples:

```text
cost 0.34309140:
    node (84,115) -> origin (21,52)

cost 0.31209809:
    node (89,98) -> origin (80,105)

cost 0.28247064:
    node (101,40) -> origin (40,83)
```

UV is similarly deletion-dominated; its highest-cost unmatched branch is:

```text
cost 0.40257373:
    node (156,148) -> origin (5,14)
```

F1 is qualitatively different. Its three highest-cost unmatched branches are
all SR-side insertions:

```text
cost 0.34857625:
    node (126,144) -> origin (142,147)

cost 0.34597051:
    node (119,33) -> origin (34,22)

cost 0.33570924:
    node (128,3) -> origin (69,114)
```

and another high-cost insertion appears at:

```text
cost 0.26316655:
    node (137,159) -> origin (132,127)
```

This spatial localization is consistent with the validated cost decomposition:
F1's slightly worse MT score vs CNN is driven by SR-side insertion cost, not
matched-branch relabeling or GT-side deletion.

### Important visual semantics

The mapped `(node, origin)` coordinates are the two critical endpoints of a
persistence branch. A straight segment between them is NOT a validated spatial
superarc trajectory.

Therefore a publication-safe diagnostic should:

```text
- mark the two endpoints,
- optionally label the exact cost/rank,
- NOT connect them with a line that could be mistaken for spatial arc geometry.
```

---

## XXXVI.145 Spatial diagnostic renderer prepared

Prepared:

```text
render_sample69_mt_cost_spatial_diagnostic.py
```

The renderer freezes the display selection before looking at the figure:

```text
top 8 unmatched branches per method and per side,
ranked only by exact TTK nonmatching cost.
```

It hard-validates:

```text
1. previous NodeId/VertexId spatial audit passed;
2. sample-69 authoritative GT arrays are exactly identical for CNN/UV/F1;
3. all selected coordinates lie inside the 160x160 patch.
```
...
```text
Figure A:
    3x2 spatial diagnostic
    CNN / UV / F1 rows
    left = GT deletion endpoints
    right = SR insertion endpoints

Figure B:
    exact squared-objective stacked components
    real-real relabel + GT deletion + SR insertion
```

No branch endpoint line is drawn.

It also writes:

```text
sample69_spatial_diagnostic_selection_top8.csv
sample69_spatial_diagnostic_summary.txt
```

so the displayed selection is frozen and auditable.

---

## XXXVI.146 Sample-69 explanatory MT analysis — CLOSED / INTERPRETABLE

The frozen spatial diagnostic and exact cost-component figure were generated
successfully.

Authoritative GT identity was revalidated:

```text
CNN GT == UV GT == F1 GT exactly for sample 69
```

The figure-selection rule was frozen before rendering:
...
Exact squared TTK merge-tree objective:
...
delta deletion  = -0.6407922551
delta insertion = +6.4708756140
delta total     = +1.9671684303
```

Thus F1 is better than CNN in:

```text
explicitly matched branch relabeling
GT-side deletion cost
```

and worse only in:

```text
SR-side insertion cost
```

The insertion increase is sufficiently large to reverse the other two
improvements and produce F1's slightly larger total MT distance.

The F1 insertion term is approximately:

```text
9.22x CNN insertion cost
12.72x matched-UV insertion cost
```

and F1 has:

```text
576 unmatched SR branch-decomposition nodes
```
...
Average insertion cost per unmatched SR branch:
...
Therefore the effect is both:

```text
more unmatched SR branches
and
larger average cost per unmatched SR branch.
```

### Concentration of nonmatching cost

Frozen top-8 coverage:

```text
CNN:
    deletion top8 = 15.20% of deletion cost
    insertion top8 = 64.79% of insertion cost

UV:
    deletion top8 = 14.61%
    insertion top8 = 57.06%

F1:
    deletion top8 = 15.78%
    insertion top8 = 29.67%
```

This adds an important observation:

> F1's insertion burden is not dominated by only a few extreme branches. Its
> top eight inserted branches explain less than one third of the insertion
> cost, so the excess insertion penalty is comparatively distributed across
> many unmatched reconstructed branches.

This is qualitatively different from CNN / UV, where the top eight insertions
explain a majority of their much smaller insertion costs.

### Relation to the threshold-3 abstract hierarchy

Frozen threshold-3 display hierarchy:

```text
GT:
    58 nodes, 57 arcs, max depth 19

CNN:
    12 nodes, 11 arcs, max depth 6

Matched UV:
    18 nodes, 17 arcs, max depth 9

F1:
    38 nodes, 37 arcs, max depth 16
```

Therefore the threshold-3 display and numerical threshold-0 cost analysis are
consistent but complementary:

```text
F1 restores substantially more surviving hierarchy than CNN / UV,
yet much of its additional threshold-0 branch structure is unmatched by GT
under the exact TTK edit objective.
```

Do NOT translate this into "spurious physical features" without independent
physical validation.

Preferred wording:

> Candidate F1 recovers substantially richer surviving Join-Tree hierarchy
> than the CNN or reconstruction-only control. However, exact decomposition of
> the numerical TTK merge-tree objective shows that F1's slightly larger
> distance than CNN is driven entirely by SR-side insertion cost. F1 actually
> reduces both matched-branch relabeling and GT-side deletion costs. The
> insertion penalty is broad rather than being dominated by a few outliers,
> indicating that additional reconstructed branch structure is not consistently
> paired with GT by the optimal merge-tree edit matching.

### Figure-role recommendation

Recommended manuscript / paper figure roles:

```text
Primary explanatory figure:
    exact squared-objective component chart

Primary qualitative companion:
    abstract threshold-3 hierarchy without NodeId labels

Supplement / diagnostic:
    top-8 unmatched-branch spatial endpoint figure

Audit-only:
    abstract hierarchy with NodeId labels

Supplement / exploratory:
    spatially embedded tree overlays and tree-only geometry figures
```

The spatially embedded tree figures remain visually cluttered and sampled arc
segments must not be interpreted as abstract-tree junctions.

### Publication polish recommendations

For the cost-component chart:

```text
- explicitly label numerical MT persistence threshold = 0;
- state that bar height is d_MT^2;
- optionally annotate d_MT above each bar;
- rename "real-real relabel" to "matched-branch relabel";
- rename "GT deletion" to "GT-side deletion";
- rename "SR insertion" to "SR-side insertion".
```

For the abstract hierarchy:

```text
- use the non-NodeId version;
- use neutral/uniform edge styling unless edge colors gain a defined semantic;
- preserve the statement that horizontal placement has no scalar/physical
  meaning;
- retain display persistence threshold = 3.0 in the title/caption.
```

For the spatial diagnostic:

```text
- keep endpoint-only markers;
- never connect persistence-pair endpoints with straight lines;
- emphasize that the displayed top-8 were selected numerically before
  rendering.
```
